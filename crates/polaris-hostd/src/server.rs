...
/// Write the bearer token to `path` with owner-only permissions (0600 on unix).
...
fn serve_unix(socket: &std::path::Path, state: Arc<AppState>) -> io::Result<()> {
    ...
    // 0660: owner and group (the dashboard container's group) may talk to it,
    // nothing else. The bearer token is still required on every request.
