...
| ---- | ---------- | ------- |
| `fjrg2007.com.polaris-zone.json` | a zone label, or the domain itself | `plr.example.com` and `*.plr.example.com`, or `example.com` and `*.example.com` |
| `fjrg2007.com.polaris-service.json` | one hostname | `app.example.com`, or `example.com` |
...
2. **The public half in DNS**, as a TXT record at `_dcpubkeyv1.dc.fjrg2007.com`,
   valued `p=1,a=RS256,d=<base64 public key>`. Split across records with `p=2`,
   `p=3` if it does not fit in one.
3. **A signing endpoint.** Cloudflare verifies a signature over the apply URL's
...
changing it means new templates and deprecating the old ones. It is set to a
domain the project controls rather than a GitHub Pages address, which cannot carry
the TXT record step 2 needs.
