...
about _any_ object answers it about exactly one kind:
...
- **There was nowhere to say anything.** A server, a runner job, a vault item and
  ...
  Polaris with a comment box was a task. _Fixed for services and servers; the
  rest are a line in `SUBJECTS` and a panel each._
- **Following something was per-app or absent.** Nothing decided who heard about
  a service, so a failed deploy told its owner and stopped there. _Fixed._
- **Every table is somebody's first table.** Drive, Servers, Runners, Firewall
  ...
  all three, and only for itself. _Still true._
...
1.  **Activity - done.** `lib/activity/activity.ts` over one `Activity` table
    ...
    renaming a box or saying where it lives leaves a line.
    `components/activity-feed.tsx` draws all three; each app hands in its own
    wording, because "moved it from Doing to Done" and "changed the PORT
    variable" are the same row and different sentences.
...
4.  **Saved views - not yet, on purpose.** Which rows, grouped how, sorted how,
    ...
    It is not being done yet because there is nothing to generalise it _for_.
...
same problem generically. What is worth taking is the _shape_ above, not the
