...
export function markdownPasteHtml(text: string): string | null {
  ...
  try {
    // `async: false` makes this synchronous at runtime; the version of `marked`
    // this repository has types the return as `string | Promise<string>`
    // regardless, so the narrowing the option already guarantees is stated.
    return marked.parse(normalized, { gfm: true, breaks: false, async: false }) as string
  } catch {
    return null
  }
}
