...
import type { WordArtPreset } from "@polaris/genoffice-ui";
...
function collectBodyBulletChars(
    text:
        | {
              lines: Array<{
                  runs: Array<{ text: string; isBullet?: boolean }>;
                  paraStart?: boolean;
              }>;
          }
        | undefined,
...
function collectBulletChars(node: RenderNode, out: Set<string>) {
    if (node.type === "shape" || node.type === "text") collectBodyBulletChars(node.text, out);
        ...
        for (const child of node.children) collectBulletChars(child, out);
}
...
function collectAligns(node: RenderNode, out: Set<ParaAlign>) {
    if (node.type === "shape" || node.type === "text") collectBodyAligns(node.text, out);
        ...
        for (const cell of node.cells) collectBodyAligns(cell.text, out);
...
export function App() {
    ...
    const [hoverAnim, setHoverAnim] = useState<{ nonce: number; items: AnimationItem[] } | null>(
        null
    );
    ...
    const previewZoom = useCallback(
        (next: number | ((current: number) => number), anchor?: { x: number; y: number }) => {
            ...
            const target =
                typeof next === "function" ? next(g.pending ?? zoomLiveRef.current) : next;
            ...
            if (!g.raf) {
                g.raf = requestAnimationFrame(() => {
                    ...
                    window.dispatchEvent(
                        new CustomEvent(ZOOM_PREVIEW_EVENT, { detail: g.pending })
                    );
                });
            }
    ...
    const askTargets = useMemo((): AskTarget[] => {
        ...
        return askState.ids.flatMap((id) => {
            ...
            return node
                ? [{ id: anchorId(node), sourceId: node.sourceId, desc: describeNode(node) }]
                : [];
        });
    ...
    const commitAsk = useCallback(
        (instruction: string) => {
            ...
            setEditQueue((prev) => {
                if (state.itemKey) {
                    return prev.map((it) =>
                        it.key === state.itemKey ? { ...it, instruction } : it
                    );
                }
    ...
    const insertModel3dFile = useCallback(
        () => insertActions.insertModel3dFile(ctxRef.current),
        []
    );
    const toggleScreenRecord = useCallback(
        () => insertActions.toggleScreenRecord(ctxRef.current),
        []
    );
    ...
    const previewTransitionOnCanvas = useCallback((kind: TransitionKind) => {
        const concrete =
            kind === "random"
                ? PREVIEWABLE_TRANSITIONS[
                      Math.floor(Math.random() * PREVIEWABLE_TRANSITIONS.length)
                  ]!
                : kind;
    ...
    const applyAnimation = useCallback(
        (effect: AnimEffectKind | "none") =>
            animationActions.applyAnimation(ctxRef.current, effect),
        []
    );
    ...
    const saveRehearseTimings = useCallback(
        () => showActions.saveRehearseTimings(ctxRef.current),
        []
    );
    ...
    const moveSlideTo = useCallback(
        (from: number, insertAt: number) =>
            slideActions.moveSlideTo(ctxRef.current, from, insertAt),
        []
    );
    ...
    const editNode = useMemo(() => {
        ...
        return slide.nodes.find((n) => n.sourceId === editing.sourceId) as
            | ShapeRenderNode
            | undefined;
    }, [editing, slide]);
    ...
    const startEdit = useCallback(
        (sourceId: string, caret?: { x: number; y: number }) => {
            ...
            setEditing({
                sourceId,
                caret,
                ...(isChild ? { groupId: enteredGroupNode!.sourceId } : {})
            });
        },
        [enteredGroupNode]
    );
    ...
    const mediaPlayNode = useMemo(
        () =>
            mediaPlay
                ? (slide?.nodes.find((n) => n.sourceId === mediaPlay.sourceId) ?? null)
                : null,
        [mediaPlay, slide]
    );
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            setSelFont((v) =>
                v && v.family === family && v.sizePt === sizePt ? v : { family, sizePt }
            );
        };
        update();
    ...
    const contextElementType = useMemo((): ContextElementType => {
        ...
        if (selectedIds.length >= 2) {
            ...
            if (
                nodes.every(
                    (n) => n && (n.type === "shape" || n.type === "picture" || n.type === "group")
                )
            )
                return "mixed";
        }
    ...
    const contextChartStyle = useMemo(
        () =>
            selectedNode?.type === "chart"
                ? ((selectedNode as ChartRenderNode).styleInfo ?? null)
                : null,
        [selectedNode]
    );
    ...
    const lastFontRef = useRef<{ family: string; sizePt: number; sizeMixed?: boolean } | null>(
        null
    );
    ...
    return (
        <div className="app">
            ...
            <Ribbon
                ...
                onToggleComments={() =>
                    showComments ? setShowComments(false) : openComments(false)
                }
                    ...
                    void (async () => {
                        ...
                        for (const id of selectedIds) {
                            ...
                            if (n?.type === "shape") {
                                ...
                                for (const c of (n as GroupRenderNode).children) {
                                    if (c.type === "shape")
                                        await applyTo(c as ShapeRenderNode, n.sourceId);
                                }
                            }
                        }
                    })();
                ...
                contextShapeFill={
                    selectedNode?.type === "shape"
                        ? (selectedNode as ShapeRenderNode).fill.kind === "solid"
                            ? toPickerHex(
                                  (selectedNode as ShapeRenderNode & { fill: { color: string } })
                                      .fill.color
                              )
                    ...
                    <div className={`ai-dock${showAi && aiSettings ? "" : " collapsed"}`}>
                        ...
                        {aiSettings ? (
                            <AiPanel
                                ...
                                onQueueConsume={(keys) =>
                                    setEditQueue((prev) =>
                                        prev.filter((it) => !keys.includes(it.key))
                                    )
                                }
                            />
                        ) : (
                            ...
                            <div className="sorter-view">
                                {slides.map((s, i) => (
                                    <div
                                        ...
                                        data-tip={
                                            s.hidden
                                                ? t("appSorterHiddenTitle")
                                                : t("appSorterItemTitle")
                                        }
                                ...
                                {viewMode === "outline" ? (
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="outline-body">
                                                        ...
                                                        {o.lines.length > 5 && (
                                                            <div className="outline-line outline-more">
                                                                …
                                                            </div>
                                                        )}
                                                    </div>
                                    ...
                                    showThumbs && (
                                        <>
                                            ...
                                            >
                                                {(() => {
                                                    ...
                                                    return sectionGroups.map((g, gi) => {
                                                        const collapsed =
                                                            g.id != null && collapsedSecs.has(g.id);
                                                        return (
                                                            <div
                                                                key={g.id ?? `lead-${gi}`}
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    {g.id != null && (
                                                                        <span
                                                                            className={`section-arrow${collapsed ? "" : " open"}`}
                                                                        >
                                                                            ▸
                                                                        </span>
                                                                    )}
                                            ...
                                            <div
                                                className="thumb-resizer"
                                                onPointerDown={startThumbsResize}
                                            />
                                        </>
                                    )
                                )}
                                        ...
                                        <div className="ruler-row">
                                            ...
                                            <StageRuler
                                                ...
                                                onGuidePreview={(client) => {
                                                    const pos = Math.min(
                                                        1,
                                                        Math.max(0, guideFracAt("h", client))
                                                    );
                                        ...
                                        {showRuler && (
                                            <StageRuler
                                                ...
                                                onGuidePreview={(client) => {
                                                    const pos = Math.min(
                                                        1,
                                                        Math.max(0, guideFracAt("v", client))
                                                    );
                    ...
                    is sized to the scaled dimensions to give the scroller the real extent */}
                                            <div
                                                ...
                                                style={
                                                    scaleBox
                                                        ? {
                                                              width: scaleBox.w * zoom,
                                                              height: scaleBox.h * zoom
                                                          }
                                                        : undefined
                                                }
                                            >
                        ...
                        disabled while the deck has no real content */}
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <SlideCanvas
                                                            ...
                                                            onPlayMedia={(id) =>
                                                                void startMediaPlayback(id)
                                                            }
                                                            onContextMenu={onCanvasContextMenu}
                                                            onMarqueeSelect={setSelectedIds}
                                                            onDuplicateTo={(id, dx, dy) =>
                                                                void duplicateSelected([id], dx, dy)
                                                            }
                                                            images={images}
                                                            zoom={zoom}
                                                        ...
                                                        {editing && editNode && (
                                                            <TextEditOverlay
                                                                ...
                                                                frameColor={selectionChromeColor(
                                                                    slide,
                                                                    images
                                                                )}
                                                                zoom={zoom}
                                                        ...
                                                        {inkTool !== "select" && !editing && (
                                                            <InkOverlay
                                                                ...
                                                                color={
                                                                    inkTool === "highlighter"
                                                                        ? inkHighlighter.color
                                                                        : inkPen.color
                                                                }
                                                                width={
                                                                    inkTool === "highlighter"
                                                                        ? inkHighlighter.width
                                                                        : inkPen.width
                                                                }
                                                                onCommit={(stroke) =>
                                                                    void commitInk(stroke)
                                                                }
                                                                onErase={(ids) =>
                                                                    void eraseInk(ids)
                                                                }
                                    ...
                                    {guideBubble && (
                                        <div
                                            ...
                                            style={{
                                                left: guideBubble.x + 14,
                                                top: guideBubble.y + 16
                                            }}
                                        >
                                            {guideBubble.text}
                                        </div>
                      ...
                      grabbable to pull the pane back out, like PPT's splitter. */}
                                    {!showNotes && (
                                        <div
                                            className="notes-pull-zone"
                                            onMouseDown={(e) => startNotesDrag(e, 0)}
                                        />
                                    )}
                                    {showNotes && (
                                        <div
                                        ...
                                        >
                                            ...
                                            <textarea
                                                ...
                                                placeholder={
                                                    notesHeight < 60
                                                        ? t("appNotesClickToAdd")
                                                        : t("appNotesPlaceholder")
                                                }
                                ...
                                {showBgFormat ? (
                                    ...
                                    <FormatPane
                                        ...
                                        slideSizePx={
                                            slide
                                                ? { w: slide.widthPx, h: slide.heightPx }
                                                : undefined
                                        }
                                        onTransform={onTransform}
                                        ...
                                        onEffects={(id, effects) =>
                                            sendEffects(current, id, effects)
                                        }
                        ...
                        <p className="rehearse-summary">
                            {t("appRehearseSummary", {
                                duration: formatClock(
                                    pendingRehearse.reduce((a, b) => a + b, 0) * 1000
                                )
                            })}
                        </p>
                        <div className="modal-actions">
                            <button onClick={() => setPendingRehearse(null)}>
                                {t("appRehearseDiscard")}
                            </button>
