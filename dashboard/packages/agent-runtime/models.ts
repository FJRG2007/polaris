...
export interface ModelAlias {
  ...
  /** alias key (within same provider) of the cheaper sibling reviewer should
...
export interface ProviderConfig {
  displayName: string;
  envVars: readonly string[];
  /** credentials authored only via `polaris auth <provider>` - never
...
export const providers = {
  ...
  "openai-compatible": provider({
    ...
    // heuristic (`hasPolarisStoredAuthForModel`, `validateAgentApiKey`), and a
...
/** managed credentials are authored only via `polaris auth <provider>` - they
...
/**
 * the models Polaris is willing to fund on the OSS program. an accepted OSS
 ...
 * user set in `BEDROCK_MODEL_ID` - Polaris never resolves or upgrades it.
 ...
 * Anthropic should set `POLARIS_AGENT=claude` to force the right route, or
