...
import { type AgentId, formatMcpToolRef, polarisMcpName } from "./external.ts";
...
// Review vs delta against the prior polaris review for IncrementalReview).
...
1. **Reviewed changes preamble** - a bolded \`**Reviewed changes**\` lead-in with one sentence on what was reviewed in this run (for \`IncrementalReview\`: what changed since the prior polaris review), then a bullet list of the substantive changes - short bolded title, one sentence each. A reviewer should understand the full reviewed scope from this list alone. Close the preamble with the metadata comment below.
...
Fill every field from the \`checkout_pr\` response - never count files or commits by hand. For \`IncrementalReview\`, fill \`Prior polaris review\` from \`list_pull_request_reviews\`.

\`\`\`
<!--
Polaris review metadata. These findings were written against {head_sha_short};
...
- Mode: Review (initial)   or   IncrementalReview (delta against prior polaris review)
...
- Prior polaris review: none   or   {prior_sha_short} ({prior_review_html_url})
-->
\`\`\`
...
3. **setup**: checkout or create the branch:
   ...
   - **new branch**: use \`${t("git")}\` to create a branch (\`git checkout -b polaris/branch-name\`)
...
6. Finalize. Reply + resolve are paired write actions: do BOTH or NEITHER for each thread.
   ...
   - call \`${t("report_progress")}\` with a brief summary`,
    ...
    // Review and IncrementalReview route the minimum reviewer specialists
...
${PR_SUMMARY_FORMAT}`,
    ...
    // prior Polaris review. The "issues must be NEW since the last Polaris
...
3. **incremental scope**: if \`incrementalDiffPath\` is present, read it FIRST to see what changed since the last review. this is a range-diff that isolates the net changes, filtering out base branch noise. then read the authoritative full diff end-to-end, beginning with the TOC and using its line ranges as your coverage checklist. if no incremental diff is present, start with the full-diff TOC, determine what changed since Polaris's most recent review, and complete the raw-diff read. only after establishing that authoritative scope and completing raw-diff coverage, use \`impactPath\` as an explicitly incomplete list of reference leads; it never replaces raw-diff reading or establishes coverage.

4. **prior feedback - read AND retire it**: fetch previous reviews via \`${t("list_pull_request_reviews")}\`, then call \`${t("get_review_comments")}\` on each prior Polaris review. Each thread renders as a section whose first line is a fenced tag \`comment author=<login> id=<fullDatabaseId> review=<reviewId> thread=<graphqlId>\`; section headers carry \`[RESOLVED]\` / \`[OUTDATED]\` when relevant. For every **open, Polaris-originated** thread, decide and act:

   - **Polaris-originated** means the FIRST \`comment author=...\` tag in the section is \`author=polaris[bot]\`. The \`*\` marker on individual comments is unrelated - it flags whether a comment belongs to the queried review, not whether it is the thread root.
   ...
   - **scope**: only retire Polaris-originated threads. Threads from human reviewers belong to those humans to resolve, even if the commit happened to address them.
...
8. **aggregate, draft, self-critique**: merge findings (yours + output from every specialist you dispatched); de-dup overlaps; trace each finding yourself. drop praise, style preferences, speculative/unverified claims, findings about pre-existing code unrelated to the new commits, anything not actionable, and anything that re-states prior review feedback (heuristic: if the finding's root cause lives in lines the *new commits* added or modified, it's in scope; otherwise drop). also drop **bloat-shaped findings** - proposed fixes that would add defensive checks for cases that can't happen, abstractions used once, comments restating obvious code, tests asserting tautologies, or "just-in-case" guards. subagents are fallible and bias toward recommending changes; the bar for an actionable inline comment is sound + correct + elegant. recommending a change that improves only one of the three (or degrades elegance to nominally improve correctness) makes the codebase worse, not better. To compute "lines the new commits added or modified": if \`incrementalDiffPath\` from step 2 is present, use it directly. Otherwise, take the prior Polaris review's \`commit_id\` (returned alongside each entry from \`${t("list_pull_request_reviews")}\` in step 4) and run \`git diff <prior-review-sha>..HEAD\` to isolate the lines added since that review.
...
9. **build the review body**: use the same default format as Review mode (preamble + optional cross-cutting \`### \` sections + optional \`### ℹ️ Nitpicks\`) - scoped to the **incremental delta**, not the full PR. The "Reviewed changes" bullets describe what changed since the prior polaris review (each bullet starts with a past-tense verb, e.g. \`- Extracted shared CLI runtime into a single module\`). Do NOT include a separate "Prior review feedback" checklist - that's tracked in the rolling PR summary snapshot for the next agent run, and surfacing it in the user-facing body is noise (changes that addressed prior feedback are already covered by the Reviewed-changes bullets). In some cases you may receive a complete diff for the whole PR instead of an incremental one; when this happens, determine what changed since Polaris's most recent review yourself before drafting bullets.
...
3. For substantial work - code changes across multiple files, multi-step investigations:
   ...
   - use ${polarisMcpName} MCP tools for GitHub/git operations
