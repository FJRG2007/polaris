...
export interface ToolState {
  ...
  // approval verdict recorded by create_pull_request_review: whether Polaris
  // would approve this PR (the agent's `approved` intent AND no outstanding
  // unresolved Polaris-originated review threads). drives the opt-in
  // `polaris-agent-approval` commit-status check posted at run end. distinct from
  ...
  // reader most wants from the `Polaris` check, so it is surfaced as a link in
  ...
  // true when the run's model costs are covered by the Polaris for OSS
  // program. carried into footers (incl. error comments built from toolState
  // alone) so the "via Polaris for OSS" attribution is consistent everywhere.
