...
// mcp name constant
export const polarisMcpName = "polaris";
 ...
 * claude code: mcp__polaris__select_mode
 * opencode:    polaris_select_mode
 */
export function formatMcpToolRef(agentId: AgentId, toolName: string): string {
  switch (agentId) {
    case "claude":
      return `mcp__${polarisMcpName}__${toolName}`;
    case "opencode":
      return `${polarisMcpName}_${toolName}`;
    default:
...
export interface WriteablePayload {
  ...
  /** the user's actual request (body if @polaris tagged) */
  prompt: string;
   ...
   * triggering @polaris comment is edited). rendered alongside the user's
  ...
  /**
   * pre-created `polaris` check-run, seeded `in_progress` by the server at dispatch so
