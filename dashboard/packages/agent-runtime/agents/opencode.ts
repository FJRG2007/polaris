 ...
 * generation of POLARIS's harness - never a version of OpenCode itself. The
 ...
 * Architecture:
 *
 *   1. Spawn ONE `opencode serve --port <p>` subprocess per Polaris run via
 ...
 *   - MCP Polaris server injected via `mcp.<name> = { type: "remote", url }`
 *   - ASKPASS for git auth
 *   - codex auth materialization + post-hook writeback
 *   - reviewer subagent config / model derivation
...
interface TurnAccumulator {
   ...
   * any subagent sessions dispatched during the turn (e.g. reviewer).
...
async function onPartUpdated(ctx: RunnerContext, part: Part): Promise<void> {
  ...
  if (part.type === "step-finish") {
    ...
    // undercount production cost/usage by the reviewer subagent's
...
export const opencode = agent({
  ...
  run: async (ctx) => {
    ...
    writeFileSync(
      join(opencodePluginDir, POLARIS_OPENCODE_GATE_PLUGIN_FILENAME),
    ...
    // codex auth lives at /var/lib/polaris/opencode/auth.json in CI (see codexHome.ts),
    ...
    // execution that sidesteps `bash: "deny"`. shell goes through polaris_shell,
    ...
    log.debug(`» starting Polaris (OpenCode, in-process SDK): ${cliPath}`);
    ...
    try {
      ...
      const sessionResp = await client.session.create({ title: "Polaris" });
