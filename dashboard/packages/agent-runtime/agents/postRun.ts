 ...
 * `task`-dispatched `reviewer` lens) calls `report_progress` and silences
 ...
 * `POLARIS_DISABLE_LEARNINGS_REFLECTION=1` disables only this optional nudge.
 */
...
function shouldRunReflection(mode: string | undefined): boolean {
  if (process.env.POLARIS_DISABLE_LEARNINGS_REFLECTION === "1") return false;
  if (!mode) return true;
 ...
 * agent has been writing (issue #619 in polaris/app). recurring failure
