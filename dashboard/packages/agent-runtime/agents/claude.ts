 ...
 * bypass Polaris's security boundary (the restricted MCP `shell` tool with a
...
function writeMcpConfig(ctx: AgentRunContext): string {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      mcpServers: {
        [polarisMcpName]: { type: "http", url: ctx.mcpServerUrl },
      },
    })
  );
  return configPath;
...
function writePretoolGateAssets(ctx: AgentRunContext): {
  ...
  const settingsPath = join(ctx.tmpdir, "polaris-claude-settings.json");
...
/**
 * Build the `--agents` JSON definition for the `reviewer` subagent.
 *
 ...
 */
const STOP_HOOK_GATE_URL_ENV = "POLARIS_GATE_URL";
...
export const claude = agent({
  ...
  run: async (ctx) => {
    ...
    const stopHookPath = join(ctx.tmpdir, "polaris-stop-hook.sh");
    ...
    //
    // we only force CLAUDE_CODE_USE_BEDROCK=1 when this is a Polaris-routed
    ...
    log.debug(`» starting Polaris (Claude Code): ${cliPath} ${baseArgs.join(" ")}`);
