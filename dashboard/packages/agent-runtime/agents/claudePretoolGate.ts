 ...
 * 2026-05-18 zed-industries/cloud incident (`reviewer` lens called
...
/**
 * Source written to `<ctx.tmpdir>/polaris-pretool-gate.mjs`. Plain ESM,
 ...
 */
export const CLAUDE_PRETOOL_GATE_FILENAME = "polaris-pretool-gate.mjs" as const;
...
// AUTOGENERATED by Polaris. PreToolUse hook that hard-blocks state-mutating
...
function stripMcpPrefix(toolName) {
  if (toolName.startsWith("mcp__polaris__")) return toolName.slice("mcp__polaris__".length);
  return toolName;
}
...
process.stdin.on("end", () => {
  ...
  try {
    ...
    process.stderr.write("polaris-pretool-gate: malformed stdin JSON\\n");
    process.exit(1);
  }
 ...
 * matcher `^mcp__polaris__` is treated as a regex by claude-code's
 ...
 * start of the tool name and fires for every Polaris MCP tool. We narrow
