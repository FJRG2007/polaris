 ...
 * `<XDG_CONFIG_HOME>/opencode/plugin/polaris-events.ts`. The harness already
 ...
 * `polaris_bus_event` - picked to be unmistakably ours so a future opencode
 * release that introduces a coincidentally-named event type won't collide.
 */

export const POLARIS_BUS_EVENT_TYPE = "polaris_bus_event" as const;

export const POLARIS_OPENCODE_PLUGIN_FILENAME = "polaris-events.ts" as const;

export const POLARIS_OPENCODE_GATE_PLUGIN_FILENAME = "polaris-subagent-gate.ts" as const;

/**
 * Source written verbatim to `<XDG_CONFIG_HOME>/opencode/plugin/polaris-events.ts`.
 *
 ...
 */
export const POLARIS_OPENCODE_PLUGIN_SOURCE = `// AUTOGENERATED by Polaris. do not edit; it'll be overwritten on the next run.
// surfaces opencode subagent activity that the CLI's run-loop discards. see
// action/agents/opencodePlugin.ts in polaris/app for why this exists. lives
...
// (polaris-subagent-gate.ts).

const POLARIS_BUS_EVENT_TYPE = ${JSON.stringify(POLARIS_BUS_EVENT_TYPE)};
...
export default async function polarisEventsPlugin() {
  return {
    event: async (input: {
      event: {
        type: string;
 ...
 * `<XDG_CONFIG_HOME>/opencode/plugin/polaris-subagent-gate.ts`. Installed by
...
export function buildOpencodeSubagentGateSource(deniedTools: readonly string[]): string {
  return `// AUTOGENERATED by Polaris. do not edit; it'll be overwritten on the next run.
// hard-blocks state-mutating MCP tool calls from opencode subagents (PR
// following polaris/app#796 - see action/agents/subagentToolGates.ts). see
// action/agents/opencodePlugin.ts in polaris/app for why this exists. lives
// inside the per-run tmpdir (XDG_CONFIG_HOME/opencode/plugin/), never inside
...
// opencode presents MCP tools as \`polaris_<bare>\`; strip that to compare
...
// list anyway.
function stripPolarisPrefix(toolName: string): string {
  if (toolName.startsWith("polaris_")) return toolName.slice("polaris_".length);
  return toolName;
}
...
export default async function polarisSubagentGatePlugin() {
  return {
    "tool.execute.before": async (
      ...
      const bare = stripPolarisPrefix(tool);
