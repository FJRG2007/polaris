...
import { type AgentId, hasSimilarIssues, polarisMcpName, type XrepoConfig } from "../external.ts";
...
import { addTools, type PolarisTool } from "./shared.ts";
...
function readEnvPort(): number | null {
  const rawPort = process.env.POLARIS_MCP_PORT;
  if (!rawPort) return null;
  const parsed = Number.parseInt(rawPort, 10);
  if (!Number.isInteger(parsed) || parsed <= 0 || parsed > 65535) {
    throw new Error(`invalid POLARIS_MCP_PORT: ${rawPort}`);
  }
  return parsed;
}
...
function buildCommonTools(ctx: ToolContext, outputSchema?: JsonSchema): PolarisTool[] {
  const tools: PolarisTool[] = [
...
async function tryStartMcpServer(
  ...
  const server = new FastMCP({ name: polarisMcpName, version: "0.0.1" });
