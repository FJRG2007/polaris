...
export function CommitChangesTool(ctx: ToolContext) {
  ...
  return tool({
    ...
    execute: execute(async (params) => {
      ...
      if (branch === "HEAD") {
        throw new Error(
          "HEAD is detached - create or check out a branch before committing (e.g. git checkout -b polaris/<description>)."
        );
      }
      rejectSpecialRef(branch, "branch");
 ...
 * shorthand for a merge-base diff, also safe). see [run 26545933188](https://github.com/polaris/app/actions/runs/26545933188)
...
function spillGitOutput(params: {
  ...
  if (!tempDir) throw new Error("POLARIS_TEMP_DIR not set");
