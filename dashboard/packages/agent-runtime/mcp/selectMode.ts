...
5. Then post a short note to the progress comment (e.g. "Plan has been updated in the comment above.") via \`${t("report_progress")}\` so it is not left as "Working on it...".`,
  };
}
...
// NOT the Polaris API JWT (ctx.apiToken). use ctx.githubInstallationToken here.
