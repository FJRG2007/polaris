...
import { buildPolarisFooter } from "../utils/buildPolarisFooter.ts";
import { log } from "../utils/cli.ts";
import { countLinesInRanges, getDiffCoverageBreakdown } from "../utils/diffCoverage.ts";
import { fixDoubleEscapedString } from "../utils/fixDoubleEscapedString.ts";
import { isPolaris } from "../utils/isPolaris.ts";
 ...
 * outstanding Polaris findings. distinct from REVIEW_THREADS_QUERY (a single
 ...
 * Polaris's bot. this is the full set of open Polaris findings on the PR -
 * the basis for both the never-approve-with-outstanding-issues invariant (the
 * approval gate below) and the `polaris-agent-approval` status verdict. it is a
 ...
 */
export async function countOutstandingPolarisThreads(
  ctx: ToolContext,
  pullNumber: number
): Promise<number> {
  let count = 0;
  ...
  for (let page = 0; page < 50; page += 1) {
    ...
    for (const thread of conn?.nodes ?? []) {
      ...
      if (root && isPolaris(root.author?.login)) count += 1;
    }
 ...
 * trigger). when such a run completes successfully and every Polaris-originated
 * review thread it raised is resolved, post a NEW approving review summarizing
 * the run's work.
 *
 * the verification step shares `countOutstandingPolarisThreads` with the
 ...
 * Polaris's own findings actually being addressed - never a blind post-fix
 * approval. a Fix-👍s run that only resolved a subset naturally fails the
 * outstanding-thread check and is left unapproved.
 *
 * respects `prApproveEnabled` (the repo's "Allow Polaris to approve PRs"
...
export async function approveAfterFix(ctx: ToolContext): Promise<void> {
  ...
  const outstanding = await countOutstandingPolarisThreads(ctx, pullNumber);
  if (outstanding > 0) {
    log.info(
      `skipping fix auto-approval: ${outstanding} unresolved Polaris review thread(s) still open on #${pullNumber}`
    );
    return;
  }
  ...
  // instead - the recorded verdict below is what drives the polaris-agent-approval
  // check + auto-merge, not the GitHub review event.
  const selfAuthored = isPolaris(pr.data.user?.login);
  ...
  const body =
    "> ✅ Polaris addressed all of its review feedback on this PR.\n\n" +
    (summary ||
      "all Polaris-raised review threads are resolved - no outstanding findings remain.");
  ...
  // record the verdict so the opt-in `polaris-agent-approval` status check (posted
 ...
 * visible review that no Polaris tool can retract (`edit_issue_comment` 404s
...
export const CreatePullRequestReview = type({
  ...
  approved: type.boolean
    .describe(
      "Set to true to submit as an approval. Use for `> ✅ No new issues found.` reviews where the PR is mergeable as-is and nothing in the body warrants code changes - approving also suppresses the Fix-button footer affordance so users don't dispatch a fix run on non-actionable feedback. Reserve approved: false for `> ℹ️ ...` (minor suggestions inline), `> [!IMPORTANT]` (recommended changes), and `> [!CAUTION]` (critical) reviews. Defaults to false (comment-only review). Mutually exclusive with request_changes. Approval is REJECTED while any unresolved Polaris review thread remains open on the PR (not just the latest commit's diff): resolve the threads the current code addresses (reply + resolve_review_thread) first, or submit a non-approving review if a real issue remains."
    )
    .optional(),
  ...
  return tool({
    ...
    execute: execute(
      async ({ pull_number, body, approved, request_changes, commit_id, comments = [] }) => {
        ...
        // invariant: Polaris must never approve a PR while ANY outstanding,
        // unaddressed Polaris finding remains - even on an incremental commit
        ...
        // Polaris-originated review threads (see countOutstandingPolarisThreads).
        ...
        if (approved) {
          const outstanding = await countOutstandingPolarisThreads(ctx, pull_number);
          if (outstanding > 0) {
            ...
            throw new Error(
              `cannot approve: ${outstanding} unresolved Polaris review thread(s) still open on this PR. ` +
                `approval requires every prior Polaris finding to be resolved, not just the latest commits to be clean. ` +
        ...
        const selfAuthored = isPolaris(prSnapshot.user?.login);

        // gate above guarantees approved ⇒ no outstanding Polaris threads, so
        ...
        if (event === "APPROVE" || event === "REQUEST_CHANGES") {
          if (!ctx.prApproveEnabled) {
            ...
            // polaris-agent-approval check + auto-merge, not a GitHub review that
        ...
        // the review exists, attach its url for the `Polaris` check's summary.
        ...
        // polaris-agent-approval check never anchors to a head the agent didn't
...
export async function createAndSubmitWithFooter(
  ...
  try {
    ...
    const footer = buildPolarisFooter({
