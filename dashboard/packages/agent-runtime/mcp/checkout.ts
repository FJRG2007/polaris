...
async function ensureBeforeShaReachable(params: EnsureBeforeShaParams): Promise<boolean> {
  ...
  const tempBranch = `polaris/tmp/${params.sha.slice(0, 12)}`;
  try {
...
export function CheckoutPrTool(ctx: ToolContext) {
  const runCheckout = async (pull_number: number): Promise<CheckoutPrResult> => {
    ...
    if (!tempDir) {
      throw new Error(
        "POLARIS_TEMP_DIR not set - checkout_pr must run in polaris action context"
      );
    }
    ...
    if (process.env.POLARIS_DISABLE_CHANGE_IMPACT === "1") {
      log.info("» change impact disabled by POLARIS_DISABLE_CHANGE_IMPACT");
    } else {
