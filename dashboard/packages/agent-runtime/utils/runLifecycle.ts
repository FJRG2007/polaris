...
export async function finalizeSuccessRun(input: {
  ...
  // in the progress comment. hang and generic get the `### ❌ Polaris
  ...
  // here would strand the user with a vanished "Working on it" comment
  ...
  if (input.toolState.output) {
    log.info(`::polaris-output::${Buffer.from(input.toolState.output).toString("base64")}`);
    core.setOutput("result", input.toolState.output);
  }

  // proactive approve-when-clean for the Fix-all / Fix-👍s flow: when a
  // fix_review run succeeded and every Polaris finding it raised is resolved,
  ...
  // status check so the verdict it records lands on `polaris-agent-approval`.
  ...
  if (input.result.success) {
    ...
    // bounded, experimental autonomous merge of any PR Polaris approved
  ...
  // own outcome so a harness-returned `{success: false}` posts `polaris`
