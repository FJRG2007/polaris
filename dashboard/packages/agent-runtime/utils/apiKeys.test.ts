...
describe("validateAgentApiKey - opencode", () => {
  ...
  it("blames the fetch, not the user, when stored secrets were unreadable", () => {
    ...
    ).toThrow("couldn't load your Polaris secrets");
  });
...
describe("isApiKeyAuthError", () => {
  it("matches the missing-key marker thrown by validateAgentApiKey", () => {
    expect(isApiKeyAuthError("no API key found. Polaris needs ...")).toBe(true);
  });
  ...
  it("matches direct-Anthropic 401 shapes", () => {
    ...
    expect(
      isApiKeyAuthError(
        "» Polaris result error: subtype=success, api_error_status=401, message=Failed to authenticate."
      )
    ).toBe(true);
  });
  ...
  it("matches expired-credential shapes", () => {
    ...
    expect(
      isApiKeyAuthError(
        "» Polaris session error: Your authentication token has expired. Please try signing in again."
      )
    ).toBe(true);
    expect(isApiKeyAuthError("» Polaris session error: Token refresh failed: 401")).toBe(true);
...
describe("formatApiKeyErrorSummary", () => {
  it("renders the missing-key body when the raw error contains the marker", () => {
    process.env.POLARIS_API_URL = "https://polaris.example";
    ...
    expect(msg).toContain("https://polaris.example/apps/agents/repos?repo=acme%2Frepo");
  });
  ...
  it("renders re-authenticate copy for expired OAuth credentials", () => {
    const msg = formatApiKeyErrorSummary({
      owner: "acme",
      name: "repo",
      raw: "» Polaris session error: Token refresh failed: 401",
    });
    expect(msg).toContain("OAuth credential has expired");
    expect(msg).toContain("polaris auth");
