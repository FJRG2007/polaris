...
import { countOutstandingPolarisThreads } from "../mcp/review.ts";
...
/**
 * true when some non-Polaris reviewer's latest binding verdict is
...
function hasBlockingHumanReview(reviews: ReviewVerdict[]): boolean {
  ...
  for (const review of reviews) {
    ...
    if (!login || isPolaris(login)) continue;
 ...
 * current head to GitHub's NATIVE auto-merge - Polaris acting as a full repo
 ...
 * protection to exist. We keep only the Polaris-specific pre-gates GitHub can't
 * express: armed toggles, approved-THIS-head, zero outstanding Polaris threads,
 ...
 *   6. `countOutstandingPolarisThreads === 0` - re-queried at merge time.
...
export async function autoMergeAfterApprove(ctx: ToolContext): Promise<void> {
  ...
  // attributable audit comment carrying the standard Polaris run footer.
  ...
  if (outstanding > 0) return skip(`${outstanding} unresolved Polaris thread(s)`);
  ...
  await comment(
    "> ✅ Merged autonomously after Polaris approved this PR and resolved all of its own review threads."
  );
}
