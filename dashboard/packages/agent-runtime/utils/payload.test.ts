...
describe("Inputs schema", () => {
  it("accepts prompt or prompt_file, both optional at the schema level", () => {
    ...
    expect(Inputs.assert({ prompt_file: ".github/polaris/triage.md" })).toEqual({
      prompt_file: ".github/polaris/triage.md",
    });
...
describe("JsonPayload schema", () => {
  it("requires ~polaris and version and prompt", () => {
    const result = JsonPayload.assert({
      "~polaris": true,
      version: "1.2.3",
      prompt: "test prompt",
    });
    expect(result).toMatchObject({ "~polaris": true, version: "1.2.3", prompt: "test prompt" });
    expect(() => JsonPayload.assert({})).toThrow();
    expect(() => JsonPayload.assert({ "~polaris": true })).toThrow();
  ...
  it.each([
    ...
    const input = { "~polaris": true, version: "1.2.3", prompt: "test prompt", [prop]: value };
