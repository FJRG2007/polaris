  ...
  return todos
    .map((todo) => {
      switch (todo.status) {
        ...
        case "cancelled":
          return `- ~~${todo.content}~~`;
        // Marked with text rather than the animated image upstream hotlinked:
        // the comment renders in somebody else's repository, and an <img> there
        // is a third-party request every reader of the pull request makes.
        case "in_progress":
          return `- [ ] ${todo.content} _(in progress)_`;
        case "pending":
          return `- [ ] ${todo.content}`;
        default:
