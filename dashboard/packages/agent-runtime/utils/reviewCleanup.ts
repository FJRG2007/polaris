 ...
 * `<owner>/<repo>/.github/workflows/<filename>@<ref>` (e.g. `.../polaris-agent.yaml@refs/heads/main`).
 * falls back to `polaris-agent.yml` if the env var is missing or malformed (shouldn't happen in CI).
 */
function getCurrentWorkflowFilename(): string {
  const ref = process.env.GITHUB_WORKFLOW_REF ?? "";
  const match = ref.match(/\/([^/]+)@/);
  return match?.[1] ?? "polaris-agent.yml";
}
