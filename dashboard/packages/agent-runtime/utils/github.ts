...
/** OIDC audience for Polaris API token exchanges */
...
const githubRequest = async <T>(
  ...
  const requestHeaders = {
    ...
    "User-Agent": "Polaris-Installation-Token-Generator/1.0",
    ...headers,
  };
...
export async function ensureGitHubToken(): Promise<void> {
  ...
  // different repo (e.g., runner token for polaris/app when tests
  // target polaris/test-repo).
  if (isOIDCAvailable()) {
...
export async function acquireNewToken(opts?: AcquireTokenOptions): Promise<string> {
  ...
  if (process.env.GITHUB_ACTIONS === "true") {
    throw new Error(
      "missing `permissions: id-token: write` on the Polaris workflow job.\n" +
        "\n" +
        "Polaris mints short-lived GitHub App installation tokens via OIDC and\n" +
        ...
        "    polaris:\n" +
        "      permissions:\n" +
        "        id-token: write   # mint Polaris installation tokens via OIDC\n" +
        "        contents: read    # for actions/checkout\n" +
        "\n" + "Check the workflow permissions block."
    );
  }
