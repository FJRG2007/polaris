 ...
 * sentinel stored in `resolve`. shared between POLARIS_MODEL override
 * and repo-config slug resolution so both paths get the same routing
 * semantics - without this helper, `POLARIS_MODEL=bedrock/byok` would
 ...
 *
 * priority:
 *   1. POLARIS_MODEL env var - resolved through the alias registry first,
 ...
 *      Bedrock model, change `BEDROCK_MODEL_ID`, not `POLARIS_MODEL`.
 ...
 *      models.dev specifier (same as `POLARIS_MODEL`), not auto-selected away.
...
export function resolveModel(ctx: { slug?: string | undefined }): string | undefined {
  const envModel = process.env.POLARIS_MODEL?.trim();
  if (envModel) {
...
export function resolveAgent(ctx: { model?: string | undefined }): Agent {
  ...
  const envAgent = process.env.POLARIS_AGENT?.trim();
  if (envAgent) {
    if (envAgent in agents) {
      return agents[envAgent as keyof typeof agents];
    }
    log.warning(`» unknown POLARIS_AGENT="${envAgent}" - falling through to auto-select`);
  }
