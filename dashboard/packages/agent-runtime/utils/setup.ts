...
export function createTempDirectory(): string {
  const sharedTempDir = mkdtempSync(join(tmpdir(), "polaris-"));
  process.env.POLARIS_TEMP_DIR = sharedTempDir;
 ...
 *     polaris/agent-runtime/get-installation-token);
 ...
 * polaris's `result` output, `post:` state handoff, and job summary.
 *
...
export async function configureRepoGit(params: ConfigureRepoGitParams): Promise<void> {
  ...
  try {
    ...
    if (shouldSetDefaults) {
      execSync('git config --local user.email "226033991+polaris[bot]@users.noreply.github.com"', {
        cwd: repoDir,
        stdio: "pipe",
      });
      execSync('git config --local user.name "polaris[bot]"', {
        cwd: repoDir,
        stdio: "pipe",
      });
