...
export async function installFromNpmTarball(params: InstallFromNpmTarballParams): Promise<string> {
  ...
  if (!tempDir) throw new Error("POLARIS_TEMP_DIR is not set");
...
export async function installFromGithub(params: InstallFromGithubParams): Promise<string> {
  // use a deterministic subdir in POLARIS_TEMP_DIR so repeated calls are cached
  const polarisTemp = process.env.POLARIS_TEMP_DIR;
  const installDir = polarisTemp
    ? join(polarisTemp, `github-${params.owner}-${params.repo}`)
