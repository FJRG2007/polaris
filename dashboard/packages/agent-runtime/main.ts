...
import { installCodexAuth, POLARIS_DATA_DIR } from "./utils/codexHome.ts";
...
export async function main(): Promise<MainResult> {
  ...
  const usageSummaryPath = process.env.POLARIS_USAGE_SUMMARY_PATH;
  if (usageSummaryPath) {
  ...
  // POLARIS_TEMP_DIR (set as a side effect of createTempDirectory) when
  ...
  // `mkdtempSync`s a NEW directory and overwrites POLARIS_TEMP_DIR, and the
  ...
  try {
    ...
    // a subsidised run is Polaris's spend, so it is pinned to `high` on
    ...
    // before Polaris installed bun itself. see #1121.
    ...
    // successful run to "❌ Polaris failed" before the agent even starts.
    ...
    const agentPromise = agent.run({
      ...
      tmpdir,
      subagentDeniedTools,
      // POLARIS_DATA_DIR (/var/lib/polaris) holds codex auth.json + any
      // future polaris-managed on-disk secrets. bash via MCP tmpfs-overlays
    ...
    if (toolContext) {
      ...
      // failed/timed-out run → post `polaris` = failure (and `polaris-agent-approval`
