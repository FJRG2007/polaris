...
/**
 * How a number is rendered against a spreadsheet number format.
 *
 * Supplied by the app rather than imported here. The original reached into
 * `@univerjs/core` for this one call, which would make this package - which has
 * two dependencies and is imported by everything - depend on a whole spreadsheet
 * framework, and drag its Node build into every browser bundle that touches a
 * chart. The app has that framework already and registers it once.
 *
 * With none registered, a label is the number as it was written. That is the
 * same answer this function already gave whenever the format could not be
 * applied, so nothing new can go wrong for want of it.
 */
export type NumberFormatter = (format: string, value: number) => string

let formatter: NumberFormatter | null = null

export function setNumberFormatter(next: NumberFormatter | null): void {
  formatter = next
}
