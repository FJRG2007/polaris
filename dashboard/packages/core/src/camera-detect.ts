...
export function letterboxFor(sourceWidth: number, sourceHeight: number, modelSize: number): Letterbox {
    ...
    return { scale, width: Math.trunc(sourceWidth * scale), height: Math.trunc(sourceHeight * scale) };
}
