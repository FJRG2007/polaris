...
/**
 * Apache's autoindex sort links. `?C=N;O=D` and its siblings are generated by a
 * served directory listing and by nothing else, so a request carrying one is either
 * a person walking a listing that should not exist or a tool harvesting it. Matching
 * the query rather than the path is what makes this precise: it needs no guess about
 * which directories are browsable.
 */
const AUTOINDEX_QUERIES = ["C=N;O=", "C=M;O=", "C=S;O=", "C=D;O="];

/**
 * Directory names that have no business being browsable. Paired with a trailing
 * slash, so `/backup/` is blocked and `/backup/restore-guide.pdf` is not - the point
 * is to refuse the listing, not the contents someone was legitimately linked to.
 *
 * Deliberately short. `/uploads/` and `/config/` belong on a list like this in
 * spirit, and are left off because plenty of real sites serve them; a pack that
 * breaks a working site gets switched off entirely, taking the rest with it.
 */
const UNLISTABLE_DIRS = [
    "/backup/",
    "/backups/",
    "/bak/",
    "/dump/",
...
/** Administrative front-ends that are worth finding and never worth exposing. Not
 *  PHP-specific, which is why they are their own pack rather than part of the CMS
 *  one - a Node service can be sitting behind any of these too. */
const ADMIN_PATHS = [
    "/adminer",
    "/pgadmin",
    "/phppgadmin",
...
export const WAF_PRESETS: readonly WafPreset[] = [
    ...
    },
    {
        ...
        rules: [
            {
                name: "Autoindex sort links",
                enabled: true,
                action: "block",
                conditions: [{ field: "query", operator: "contains", values: AUTOINDEX_QUERIES }]
            },
            {
                name: "Browsing a directory that should not be listed",
                enabled: true,
                action: "block",
                conditions: [
                    { field: "path", operator: "contains", values: UNLISTABLE_DIRS },
                    { field: "path", operator: "ends_with", values: ["/"] }
                ]
            }
        ]
    },
        ...
        rules: [
            {
                ...
                enabled: true,
                action: "block",
                conditions: [{ field: "path", operator: "contains", values: ADMIN_PATHS }]
            }
        ]
    },
    {
