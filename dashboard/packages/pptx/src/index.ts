...
function parseSlideFromArchive(archive: PackageArchive, slidePath: string): Slide | null {
    ...
    const layoutXml =
        (chain.layoutPath ? archive.readText(chain.layoutPath) : undefined) ?? undefined;
    const masterXml =
        (chain.masterPath ? archive.readText(chain.masterPath) : undefined) ?? undefined;
    if (chain.themePath) {
...
function buildDecorations(
    ...
    if (masterXml && parts.masterPath) {
        const hfTypes = new Set(
            [...enabled].filter((k) => !slidePh.has(k) && !hasPh(layoutXml, k))
        );
    ...
    if (chain.themePath) {
        ...
        if (themeXml) {
            ...
            theme.clrMap = parseClrMap(
                masterXml,
                layoutXml,
                archive.readText(slidePath) ?? undefined
            );
        }
    }
    const presXml = archive.readText("ppt/presentation.xml");
...
function validAdjust(adjust: Record<string, number>): boolean {
    ...
    return (
        entries.length > 0 &&
        entries.every(([n, v]) => /^[A-Za-z0-9]+$/.test(n) && Number.isFinite(v))
    );
}
...
function swapGeometryXml(xml: string, prst: string): string | null {
    ...
    if (existing)
        return (
            xml.slice(0, existing.index) + geomXml + xml.slice(existing.index + existing[0].length)
        );
...
export function setElementEffects(slide: Slide, elementId: string, patch: EffectsPatch): boolean {
    ...
    if (patch.shadow !== undefined) {
        ...
        if (patch.shadow !== null) {
            ...
            if (s.inner) {
                children.set(
                    "innerShdw",
                    `<a:innerShdw${base}>${effectColorXml(s.color)}</a:innerShdw>`
                );
            } else {
    ...
    if (patch.glow !== undefined) {
        if (patch.glow === null) children.delete("glow");
            ...
            children.set(
                "glow",
                `<a:glow rad="${rad}">${effectColorXml(patch.glow.color)}</a:glow>`
            );
        }
    }
    ...
    if (patch.softEdge !== undefined) {
        if (patch.softEdge === null) children.delete("softEdge");
            ...
            children.set(
                "softEdge",
                `<a:softEdge rad="${Math.max(0, Math.round(patch.softEdge))}"/>`
            );
    }
    ...
    if (patch.glow !== undefined) {
        if (patch.glow === null) delete eff.glow;
            ...
            eff.glow = {
                color: patch.glow.color,
                radius: Math.max(0, Math.round(patch.glow.radius))
            };
    }
    if (patch.reflection !== undefined) {
...
export function setElementImageFill(
    ...
    if (grp && child) {
        if (!patchGroupChildXml(grp, child, (xml) => patchElementFill(xml, { rawFillXml })))
            return null;
        (child as TextElement).fill = model;
    } else if (el) {
...
export function setPictureOpacity(slide: Slide, sourceId: string, opacity: number): boolean {
    ...
    let xml = patchedElementXml(el);
    xml = xml.replace(
        /<a:alphaModFix\b[^>]*\/>|<a:alphaModFix\b[^>]*>[\s\S]*?<\/a:alphaModFix>/,
        ""
    );
...
export function replacePictureBytes(
    ...
    if (/r:embed="/.test(tag))
        tag = tag
            .replace(/r:embed="[^"]*"/, `r:embed="${added.rid}"`)
            .replace(/\s+r:link="[^"]*"/, "");
...
function registerNewSlide(opened: OpenedPptx, sourceIndex: number, newPath: string): Slide | null {
    ...
    if (ct) {
        ...
        archive.entries.set(
            ctPath,
            Buffer.from(ct.replace("</Types>", `${override}</Types>`), "utf8")
        );
    }
...
export function mergeSlideFromSource(target: OpenedPptx, source: MergeSlideSource): Slide | null {
    ...
    const layoutTarget = anchorSlide
        ? [...archive.readRels(anchorSlide.path).values()].find((r) =>
              r.type.endsWith("/slideLayout")
          )?.target
        : undefined;
...
export function updateConnectorsForMoved(slide: Slide, movedIds: string[]): number {
    ...
    for (const el of slide.elements) {
        ...
        if (
            !(
                (cxn.start && movedSpids.has(cxn.start.id)) ||
                (cxn.end && movedSpids.has(cxn.end.id))
            )
        )
            continue;
...
export function editTableCellText(
    ...
    const lstStyle =
        /<a:lstStyle\b(?:[^>]*?)(?:\/>|>[\s\S]*?<\/a:lstStyle>)/.exec(inner)?.[0] ?? "";
    const paras = (paragraphs.length ? paragraphs : [{ runs: [{ text: "" }] }])
        .map((p) => generateParagraphXml(p))
        .join("");

    const newTc =
        tcXml.slice(0, txOpen.index + txOpen[0].length) +
        bodyPr +
        lstStyle +
        paras +
        tcXml.slice(txEnd);
...
export function editChartElement(
    ...
    if (patch.switchRowCol) {
        ...
        series = cats.map((cat, ci) => ({
            name: cat,
            values: series.map((s) => s.values[ci] ?? 0)
        }));
    ...
    const legendPos =
        patch.legendPos ??
        (existing.legendPos == null
            ? "none"
            : existing.legendPos === "tr"
              ? "r"
              : existing.legendPos);
...
export function getChartElementData(
    ...
    return {
        ...
        pointColors: chartEl.chart.series.map((s) =>
            s.pointColors ? [...s.pointColors] : undefined
        )
    };
}
        ...
        for (const el of slide.elements) {
            ...
            if ((el.type === "text" || el.type === "shape") && (el as TextElement).text) {
                ...
                for (const child of (el as GroupElement).children) {
                    if (budget <= 0) break;
                    if (
                        (child.type === "text" || child.type === "shape") &&
...
export function mergeTableCells(
    ...
    if (op.kind === "merge-right") {
        ...
        tr2Xml =
            tr2Xml.slice(0, target.start) +
            toCovered(targetXml, "vMerge") +
            tr2Xml.slice(target.end);
...
export function pasteElements(
    ...
    for (const rel of archive.readRels(slide.path).values()) {
        const abs =
            rel.targetMode === "External" ? rel.target : resolveTarget(slide.path, rel.target);
    ...
    const xmls = items.map((item) => {
        ...
        for (const rel of item.rels) {
            ...
            if (rid !== rel.rid) {
                xml = xml.replace(
                    new RegExp(`\\br:(embed|link|id)="${rel.rid}"`, "g"),
                    `r:$1="${rid}"`
                );
            }
        }
...
export function patchGroupChildXml(
    ...
    grp.anchor.originalXml =
        grpXml.slice(0, slice.start) + patch(slice.xml) + grpXml.slice(slice.end);
    return true;
}
...
export function editGroupChildStroke(
    ...
    if (!patchGroupChildXml(found!.grp, child, (xml) => patchElementStroke(xml, stroke)))
        return false;
    slide.structureDirty = true;
    return true;
}
