...
describe("what the project printed", () => {
    ...
    it("recognizes nixpacks having nothing to start", () => {
        expect(diagnoseDeploy(log("Error: No start command could be found"))?.cause).toBe(
    ...
    it("reads yarn's engine refusal", () => {
        const found = diagnoseDeploy(
            log(
                'error astro@5.1.0: The engine "node" is incompatible with this module. Expected version ">=22.12.0". Got "22.3.0"'
            )
        );
    ...
    it("builds on the Python version pip says the project needs", () => {
        const found = diagnoseDeploy(
            log("ERROR: Package 'api' requires a different Python: 3.9.18 not in '>=3.11'")
        );
    ...
    it("generates the secret Laravel, Rails and Django refuse to start without", () => {
        ...
        expect(
            diagnoseDeploy(
                log("ArgumentError: Missing `secret_key_base` for 'production' environment")
            )?.fix
        ).toMatchObject({ name: "SECRET_KEY_BASE", generate: true });
        expect(
            diagnoseDeploy(
                log(
    ...
    it("says which tool the build could not find", () => {
        const found = diagnoseDeploy(
            log("> web@0.1.0 build", "> vite build", "sh: 1: vite: not found")
        );
    ...
    it("tells a missing dependency from missing build output", () => {
        ...
        expect(
            diagnoseDeploy(log("Error: Cannot find module '/workspace/dist/server.js'"))?.fix
        ).toEqual({ kind: "set-start-command", suggestion: null });
        expect(
            diagnoseDeploy(log("ModuleNotFoundError: No module named 'gunicorn'"))?.title
        ).toContain("gunicorn");
    });

    it("points the service at the port the app actually listens on", () => {
        const found = diagnoseDeploy(log("> start", "Server listening on port 8080"), {
            port: 3000
        });
    ...
    it("sets HOST when the app only listens on localhost", () => {
        const found = diagnoseDeploy(
            log("  Local:   http://localhost:4321/", "Server running on http://localhost:4321")
        );
        expect(found?.fix).toEqual({
            kind: "add-variable",
            name: "HOST",
...
describe("what it leaves alone", () => {
    it("does not blame the lockfile when the install already fell back past it", () => {
        ...
        expect(diagnoseDeploy(log("ERR_PNPM_OUTDATED_LOCKFILE  Cannot install"))?.cause).toBe(
            "lockfile-mismatch"
        );
    });
