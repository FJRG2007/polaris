/**
 * The one place the control plane borrows SSH connections from, so an operation on
 * a machine pays for a handshake only when there is no connection to reuse.
 *
 * Why this exists: a Drive listing, a metrics probe or a Docker call is a few
 * milliseconds of work behind a TCP + key exchange + auth handshake that costs
 * hundreds. Opening one per request made browsing a server feel slow next to the
 * same server in any desktop SFTP client, which logs in once and keeps the session.
 *
 * Two things are borrowed here, not one:
 *
 *   - The connection, from `SshPool`. Leases keep a connection alive while anything
 *     is using it (a long download must not be reaped mid-transfer) and idle ones
 *     are closed after the pool's TTL, so a machine nobody is looking at is not
 *     holding a session open forever.
 ...
 *
 * Connections are keyed by purpose AND machine. One connection per machine would be
 * tidier but sshd allows a bounded number of concurrent channels on a connection
...
import { openSftp, openSshClient, SshPool, type SshConnectOptions, type SshLease } from "@polaris/ssh";
