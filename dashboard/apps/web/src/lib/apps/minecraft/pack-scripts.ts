...
export function shellInstaller(manifestUrl: string, server: string): string {
    return `#!/bin/sh
# Installs the mods for "${server}" into this machine's Minecraft folder.
...
# player added themselves is not in the record and is left alone.
removed=0
if [ -f "$record" ]; then
...
export function powershellInstaller(manifestUrl: string, server: string): string {
    return `# Installs the mods for "${server}" into this machine's Minecraft folder.
# Run it again to update. It only ever touches jars it installed itself.
...
# Only what this pack installed before and the server no longer lists.
$removed = 0
...
Set-Content -LiteralPath $record -Value $wanted -Encoding utf8
...
export function packTable(mods: readonly { filename: string; sha1: string; url: string }[]): string {
    return mods.map((mod) => `${mod.filename}\t${mod.sha1}\t${mod.url}\n`).join("");
}
