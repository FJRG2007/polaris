/**
 * Reading and writing the ARK server's own files inside its container.
 *
 ...
 * ARK has no way to be asked about any of them over RCON. So they are read and
 * written where they live.
 *
 * Through the container's own shell rather than through the daemon's file API:
 * that one only reaches containers on the local host, and a game server registered
 * on another machine is exactly the case that has to keep working. `run` already
 * works on both, so a write is a base64 blob handed to `base64 -d` - the bytes
 * never touch a shell as text, and the only thing interpolated into the command is
 * a path this module owns and a string of base64 characters.
 *
 * A write lands in a temporary file first and is then poured into the real one, so
 * a half-written settings file is never what the server boots from, and the file
 * keeps whatever ownership it already had - it is read by the game's own account,
 * not by the one this command runs as.
 */
...
/** Paths are this module's callers' own constants, never anything typed. Proved
 *  rather than assumed, because everything below puts one in a shell command. */
...
export async function readArkFile(server: ServerContainer, path: string): Promise<string | null> {
    assertSafePath(path);
 ...
 */
export async function writeArkFile(server: ServerContainer, path: string, content: string): Promise<void> {
    assertSafePath(path);
