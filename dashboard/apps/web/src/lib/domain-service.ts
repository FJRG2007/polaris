...
export async function getDomainConfig(): Promise<DomainConfig> {
    const [app, sharing, sub, token] = await Promise.all([
        getSetting(KEYS.app),
...
/** Base URL for share links and drop points: sharing domain, else app domain, else env. */
export async function sharingBaseUrl(): Promise<string> {
    return (
        normalizeUrl(await getSetting(KEYS.sharing)) ??
        normalizeUrl(await getSetting(KEYS.app)) ??
        loadEnv().POLARIS_APP_URL
    );
}
