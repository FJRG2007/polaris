 ...
 *
 * The verdict is deliberately three-valued. Only a provider explicitly saying
 * "not this key" refuses the save; a timeout, a 500, or an endpoint that has
...
/** One provider's read-only probe. */
interface Probe {
    url: string;
 ...
 * its own key header, xAI publishes an endpoint that describes the key, and
 * OpenRouter's model list is public - checking a key against it would accept
 * every string ever pasted.
 */
const PROBES: Record<string, Probe> = {
    ...
    openrouter: {
        // The model list is public there, so it would accept anything. This one
        // describes the key, and nothing else does.
...
export async function checkProviderKey(provider: string, secret: string): Promise<KeyCheck> {
    if (provider === GATEWAY_SLUG) {
        return { state: "unverified", reason: "An endpoint of your own is proven by the first run, not from here." };
    }
    const probe = PROBES[provider];
    if (!probe) return { state: "unverified", reason: "Polaris has no way to check this provider's keys." };

    let response: Response;
    try {
        response = await fetch(probe.url, {
            method: "GET",
            headers: probe.headers(secret.trim()),
            signal: AbortSignal.timeout(TIMEOUT_MS),
            cache: "no-store"
        });
    } catch {
        return { state: "unverified", reason: "Could not reach the provider to check the key." };
    }
...
export function classify(status: number): KeyCheck {
    ...
    if (status === 401 || status === 403) {
        return { state: "rejected", reason: "The provider did not accept that key." };
    }
    if (status === 429) {
        return { state: "unverified", reason: "The provider is rate limiting this account, so the key could not be checked." };
    }
    return { state: "unverified", reason: `The provider answered ${status}, so the key could not be checked.` };
}
