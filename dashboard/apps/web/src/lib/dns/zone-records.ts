...
export async function zoneRecords(scope: DnsScope): Promise<ZoneRecords> {
    ...
    return {
        ...
        within: zone.within,
        records: records.map((record) => viewOf(record, zone.name))
    };
}
...
export async function saveZoneRecord(
    ...
    if (!inside(zone, checked.record.name)) {
        throw new DnsEditError(
            `Only names at or under ${zone.within ?? zone.name} can be edited here`,
            {
...
export async function recordPropagation(
    ...
    if (!(DNS_RECORD_TYPES as readonly string[]).includes(current.type)) {
        throw new DnsEditError(
            "Propagation is checked for A, AAAA, CNAME, TXT, MX, SRV and CAA records"
        );
    }
