...
import type { MachineResources, RunnerIsolation } from "@polaris/core";
...
export class LocalRunnerHost implements RunnerMachine {
    ...
    private async startProject(
        input: { name: string; release: RunnerRelease; jitConfig: string },
                ...
                services: [
                    {
                        ...
                        env: { ACTIONS_RUNNER_INPUT_JITCONFIG: input.jitConfig },
                        ports: [],
                        volumes: [],
    ...
    /** The tail of a container's log, best-effort: a container that is already
     *  gone has no log, which is not a reason to fail the reap. */
    private async logs(container: string): Promise<string> {
        ...
        await this.ports
            ...
            .catch(() => undefined);
        return collected.slice(0, MAX_LOG).trim();
    }
}
