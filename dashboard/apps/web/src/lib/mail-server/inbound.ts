...
async function fire(
    ...
    const body = [
        ...
        ceiling.last
            ? "This rule has matched often in the last ten minutes; further matches are not notified until the window ends."
            : null
    ]
        .filter(Boolean)
        .join(" ");
...
export async function receiveEvents(
    ...
    const rules = await prisma.mailInboundRule.findMany({
...
export async function listRules(server: MailServer): Promise<InboundRuleView[]> {
    const rows = await prisma.mailInboundRule.findMany({
...
): Promise<InboundRuleView> {
    const count = await prisma.mailInboundRule.count({ where: { serverId: server.id } });
    if (count >= MAX_RULES)
        throw new MailServerAccessError(`A mail server holds at most ${MAX_RULES} rules.`);
...
export async function updateRule(
    ...
    if (updated.count === 0)
        throw new MailServerAccessError("That rule is not on this mail server.");
    await recordAudit({
        actorId,
        action: "mailserver.rule.update",
        targetType: "mail-server",
        targetId: server.id,
        orgId: server.orgId ?? undefined
    });
...
export async function deleteRule(
    ...
    const removed = await prisma.mailInboundRule.deleteMany({
        where: { id: ruleId, serverId: server.id }
    });
    if (removed.count === 0)
        throw new MailServerAccessError("That rule is not on this mail server.");
