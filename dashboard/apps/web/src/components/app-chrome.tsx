/**
 * The signed-in frame: the top bar, the app switcher, the rail and everything
 * that has to exist above every screen - the held call, incoming calls, message
 * toasts, the notification feed, presence, display preferences.
 *
 * A component rather than the layout itself because one page outside the
 * authenticated group needs the very same frame: a public profile, which is
 * readable without an account but must not look like a different product to
 * somebody who has one. A layout cannot be borrowed by a route that is not
 * under it, so the frame is a component and the layout is four lines.
 *
 * Everything here reads from the identity it is handed. It never resolves a
 * session of its own, so the two callers keep their own answers to what happens
 * when there is not one: the group redirects, the profile draws the public bar.
 */
 ...
 * correctly for the running edition. The notification feed is seeded here too,
 * so the bell badge is right on the first paint rather than after the live
 * stream connects, and the user's display preferences are resolved once here
 * rather than per screen. The same goes for the address Polaris is reachable at:
 * every screen that hands out a link builds it on the configured domain instead
 * of the hostname of the tab. Where the reader has been is noted here as well,
