 ...
 *
 * Everything is served as a download, and that is not laziness. This is content
 * from outside: an HTML part or an SVG rendered inline would be somebody else's
 * markup running on this origin, which is the oldest way there is to steal a
 * session. Pictures the message itself draws never come through here - they are
 * inside the message body, which is drawn in a sandboxed frame.
 */
...
export async function GET(
    ...
    try {
        ...
        return new Response(new Uint8Array(file.bytes), {
            headers: {
                // Deliberately not the part's own type. See the note above: this
                // is a file from outside, and it is handed over to be saved
