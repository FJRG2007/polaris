/**
 * Check every running service for a newer image or commits it does not have, on demand.
 *
 * Polaris' own scheduler already does this every half hour (see `lib/cron`); the
...
export async function POST(request: Request): Promise<Response> {
    ...
    const result = await runScheduledJob("update-scan");
