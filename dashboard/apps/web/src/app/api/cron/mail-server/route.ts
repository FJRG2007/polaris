/**
 * Check the mail servers Polaris runs and file the DMARC reports they received -
 * on demand.
 *
 * Polaris' own scheduler runs this every quarter of an hour (see
 * `lib/cron/scheduler`). The route stays for an operator who would rather drive
...
export async function POST(request: Request): Promise<Response> {
    ...
    const result = await runScheduledJob("mail-server");
