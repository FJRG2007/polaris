...
import {
    ...
    uploadTarget,
    uploadedReleaseMetaSchema
} from "@/lib/deploy/api/uploaded-release";
...
export const POST = deployRoute(
    "deploy the uploaded image",
    true,
    async ({ caller, request, url, params }) => {
        const declared = Number(request.headers.get("content-length"));
        if (Number.isFinite(declared) && declared > MAX_SHIPPED_BYTES)
            throw new DeployApiRefusal(413, TOO_LARGE);
        ...
        try {
            ...
            const result = await deployUploadedRelease(
                caller,
                params.id ?? "",
                { file, bytes },
                meta
