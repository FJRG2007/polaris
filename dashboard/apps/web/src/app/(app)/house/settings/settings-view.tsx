"use client";

/**
 * How the house is set up: where footage goes, and what does the recognizing.
 *
 * Two settings, and both of them are decisions somebody makes once. Storage is
 * shared with the rest of Polaris on purpose - a house that has a NAS should not
 * have to be told about it twice - and it is re-read on every write, so
 * connecting one later moves new footage onto it with no migration.
 */

import * as actions from "../actions";
import { useEffect, useState } from "react";
import { runAction } from "@/lib/run-action";
import { CircleCheck, Loader2 } from "lucide-react";
import { Button, Input, Select, Skeleton } from "@polaris/ui";

interface Settings {
    hasFaceKey: boolean;
    recognizerInstalled: boolean;
}

export function HomeSettingsView({
    storage,
    targets
}: {
    /** What footage is written to today. */
    storage: string;
    targets: { id: string; label: string }[];
}) {
    const [settings, setSettings] = useState<Settings | null>(null);
    const [target, setTarget] = useState(storage);
    const [key, setKey] = useState("");
    const [savingKey, setSavingKey] = useState(false);
    const [savedKey, setSavedKey] = useState(false);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        let cancelled = false;
        void (async () => {
            const result = await actions.homeSettingsAction();
            if (cancelled) return;
            if (result.error) setError(result.error);
            setSettings(result.settings ?? { hasFaceKey: false, recognizerInstalled: false });
        })();
        return () => {
            cancelled = true;
        };
    }, []);

    const saveTarget = async (next: string) => {
        const previous = target;
        setTarget(next);
        const result = await runAction(() => actions.setHomeStorageAction(next), setError);
        if (result?.error) {
            setError(result.error);
            setTarget(previous);
        }
    };

    const saveKey = async () => {
        setSavingKey(true);
        setSavedKey(false);
        const result = await runAction(() => actions.setFaceKeyAction(key), setError);
        setSavingKey(false);
        if (result?.error) {
            setError(result.error);
            return;
        }
        setKey("");
        setSavedKey(true);
        setSettings((current) => (current ? { ...current, hasFaceKey: true } : current));
    };

    return (
        <div className="flex flex-col gap-6">
            <section className="flex flex-col gap-2">
                <div>
                    <h2 className="text-[13px] font-semibold text-foreground">Where footage goes</h2>
                    <p className="mt-0.5 text-[12px] leading-relaxed text-muted-foreground">
                        Clips and the pictures that go with events. A NAS is the right answer if you have one - it is
                        the disk with the room and the backups. Changing this moves new footage only; what is already
                        stored stays readable where it is.
                    </p>
                </div>
                <Select
                    value={target}
                    onValueChange={saveTarget}
                    className="w-72"
                    aria-label="Where footage goes"
                    options={targets.map((option) => ({ value: option.id, label: option.label }))}
                />
            </section>

            <section className="flex flex-col gap-2">
                <div>
                    <h2 className="text-[13px] font-semibold text-foreground">Face recognition</h2>
                    <p className="mt-0.5 text-[12px] leading-relaxed text-muted-foreground">
                        Install the recognizer from the marketplace, then paste the API key it gives you. Faces are only
                        ever sent to it when a camera has already seen a person, and they never leave the machine it
                        runs on.
                    </p>
                </div>
                {settings === null ? (
                    <Skeleton className="h-9 w-72" />
                ) : (
                    <>
                        <div className="flex flex-wrap items-center gap-2">
                            <Input
                                value={key}
                                onChange={(event) => setKey(event.target.value)}
                                className="w-72"
                                type="password"
                                autoComplete="off"
                                aria-label="Recognition key"
                                placeholder={settings.hasFaceKey ? "Stored. Type to replace it." : "Paste the key"}
                            />
                            <Button variant="secondary" onClick={saveKey} disabled={savingKey || !key.trim()}>
                                {savingKey ? <Loader2 className="size-4 shrink-0 animate-spin" /> : null}
                                Save
                            </Button>
                            {savedKey ? (
                                <span className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                                    <CircleCheck className="size-3.5 shrink-0 text-success" />
                                    Saved
                                </span>
                            ) : null}
                        </div>
                        {settings.hasFaceKey && !settings.recognizerInstalled ? (
                            <p className="text-[12px] text-muted-foreground">
                                The key is stored, but no recognizer is installed yet - add it from the marketplace and
                                it will start being used.
                            </p>
                        ) : null}
                    </>
                )}
            </section>

            {error ? <p className="text-[12px] text-danger">{error}</p> : null}
        </div>
    );
}
