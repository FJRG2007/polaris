...
export function HandStrip({ call }: { call: CallState }) {
    ...
    return (
        <div
            className="flex shrink-0 flex-wrap items-center gap-2 rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-xs"
            // Said out loud when it changes, because the whole point of the
            // gesture is to be noticed by somebody who is looking at a face and
            // listening to a voice.
            aria-live="polite"
        >
            <Hand className="size-4 shrink-0 text-warning" />
            <span className="shrink-0 font-medium">{handsSummary(hands)}</span>
