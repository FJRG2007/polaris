...
/**
 * A call with nothing in the middle.
 *
 ...
 *
 * It is the fallback rather than the way calls normally work: an instance with a
 * call server set up uses that instead - see `use-sfu-call` - and this is what
 * runs when there is none. Which still works, inside one network, without
 * anybody having to install anything.
 *
...
import { callDevices, openMedia } from "./call-media";
 ...
 */
export function useMeshCall(meetingId: string | null, options?: { video?: boolean }): CallState {
    ...
    /** What this browser has to offer, named. Only filled in once a permission
     *  has been granted, which is why this runs after the stream. */
