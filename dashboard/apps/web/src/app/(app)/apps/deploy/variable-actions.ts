...
export async function saveEnvVarChangesAction(
    ...
    try {
        ...
        if (redeploy)
            void redeployForEnvScope(scope, scopeId, access.ownerId).catch(() => undefined);
...
export async function redeployEnvScopeAction(input: unknown): Promise<{ error?: string }> {
    ...
    try {
        ...
        void redeployForEnvScope(parsed.data.scope, parsed.data.scopeId, access.ownerId).catch(
            () => undefined
        );
        revalidatePath(DEPLOY_PATH);
        return {};
