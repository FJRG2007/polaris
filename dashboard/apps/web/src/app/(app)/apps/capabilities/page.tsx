...
export default async function CapabilitiesPage() {
    ...
    const needed = [...new Set(CAPABILITY_GROUPS.flatMap((group) => group.items.flatMap((item) => item.needs ?? [])))];
    const held = new Set(
        (await Promise.all(needed.map(async (permission) => ((await sessionCan(user, permission)) ? permission : null)))).filter(
