...
export function ReportsTab({ serverId }: { serverId: string }) {
    ...
    const panel = usePanelData(`dmarc:${serverId}:${range}`, () =>
        dmarcAction({ serverId, days: Number(range) })
    );
    ...
    async function collect(): Promise<void> {
        ...
        if (answer.error) setMessage({ tone: "error", text: answer.error });
            ...
            setMessage({
                tone: "ok",
                text: `${answer.messages} message${answer.messages === 1 ? "" : "s"} read, ${answer.filed} new report${answer.filed === 1 ? "" : "s"}.`
            });
            await panel.reload();
        }
    }
    ...
    async function upload(chosen: File): Promise<void> {
        ...
        const response = await fetch(`/api/mail-server/${serverId}/reports`, {
            method: "POST",
            body
        }).catch(() => null);
        ...
        if (!response?.ok || !answer || answer.error) {
            setMessage({
                tone: "error",
                text: answer?.error ?? "The report could not be uploaded."
            });
            return;
        }
        setMessage({
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {panel.error && !overview ? (
                <PanelError message={panel.error} onRetry={() => void panel.reload()} />
            ) : null}
            {overview ? (
                <p className="text-xs text-muted-foreground">
                    Receivers send reports to <Mono>{overview.address}</Mono> and its alias at each
                    domain.{" "}
                    {overview.checkedAt
                        ? `Last read ${format.dateTime(overview.checkedAt)}.`
                        : "Not read yet."}
            ...
            {overview && overview.sources.length > 0 ? (
                <div className="overflow-x-auto">
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                        An address that always fails is either somebody sending as your domain, or a
                        real sender of yours that is not in SPF or not signing with DKIM.
                    </p>
                </div>
            ) : null}
            {overview && overview.reports.length > 0 ? (
                <details className="text-[0.8125rem]">
                    <summary className="cursor-pointer text-muted-foreground">
                        {overview.reports.length} report{overview.reports.length === 1 ? "" : "s"}{" "}
                        in this range
                    </summary>
