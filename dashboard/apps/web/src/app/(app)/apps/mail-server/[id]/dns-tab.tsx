...
type Domain = Extract<
    Awaited<ReturnType<typeof listDomainsAction>>,
    { domains: unknown }
>["domains"][number];
        ...
        <div className="flex flex-col gap-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-xs text-muted-foreground">
                    {shown?.at
                        ? `Last checked ${format.dateTime(shown.at)} against public resolvers.`
                        : "Not checked yet."}
                </p>
            ...
            {!domains.data ? (
                ...
                <EmptyState
                    icon={<Globe />}
                    title="No domains"
                    description="Add the domain this server receives mail for."
                />
            ) : null}
            {list.map((domain) => {
            ...
            <AddDomainDialog
                ...
                onAdded={() => void domains.reload()}
            />
            <CatchAllDialog
                serverId={serverId}
                domain={catchAll}
                onClose={() => setCatchAll(null)}
                onSaved={() => void domains.reload()}
            />
            <PublishDialog
                serverId={serverId}
                domain={publishing}
...
function RecordTable({ domain, report }: { domain: Domain; report: Reports[number] | null }) {
    if (!domain.zoneFile.trim()) {
        return (
            <p className="text-xs text-muted-foreground">
                The mail server has not produced this domain's records yet.
            </p>
        );
    }
    ...
    if (!rows) {
        return (
            <p className="text-xs text-muted-foreground">
                Run "Check DNS" to see each record and whether it is published.
            </p>
        );
    }
    return (
            ...
            <table className="w-full text-[0.8125rem]">
                ...
                <tbody className="divide-y divide-border">
                    {rows.map((entry, index) => (
                        <tr
                            key={`${entry.record.type}-${entry.record.name}-${index}`}
                            className="align-top"
                        >
                            <td className="py-2 pr-3">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                        {MAIL_RECORD_LABELS[
                                            entry.record.purpose as MailRecordPurpose
                                        ] ?? ""}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td className="py-2 pr-3">
                                <div className="flex flex-col gap-1">
                                    <VerdictBadge
                                        verdict={entry.checkable ? entry.verdict : "unverified"}
                                    />
                                    {entry.note ? (
                                        <span className="text-xs text-muted-foreground">
                                            {entry.note}
...
function AddDomainDialog({
    ...
    const fieldError =
        !parsed.success && name.trim() ? (parsed.error.issues[0]?.message ?? null) : null;
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-[min(28rem,95vw)] max-w-[min(28rem,95vw)]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        The server generates its signing key; its records appear here to publish.
                    </DialogDescription>
                </DialogHeader>
                <form
        ...
        >
            <DialogContent className="w-[min(28rem,95vw)] max-w-[min(28rem,95vw)]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Mail to an address nobody has at this domain goes here instead of bouncing.
                    </DialogDescription>
                </DialogHeader>
                <Field
                    label="Deliver to"
                    hint="Catch-alls attract spam; leave empty to let unknown addresses bounce."
                >
                    {(id) => (
                        <Input
                            id={id}
                ...
                <DialogFooter>
                    ...
                    <Button
                        type="button"
                        onClick={() => void save(address.trim() || null)}
...
function PublishDialog({
    ...
    async function apply(): Promise<void> {
        ...
        setApplying(true);
        setError(null);
        const answer = await applyDnsAction({
            serverId,
            domainId: domain.id,
    ...
    const changes =
        plan?.records.filter((entry) => entry.action === "create" || entry.action === "update")
            .length ?? 0;
        ...
        <Dialog open={domain !== null} onOpenChange={(open) => (open ? undefined : onClose())}>
            <DialogContent className="w-[min(48rem,95vw)] max-w-[min(48rem,95vw)]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Nothing is changed until you apply. Records that are someone else's stay
                ...
                <DialogFooter className="flex-wrap items-center gap-3">
                    {plan && !results && conflicts > 0 ? (
                        <label className="mr-auto flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            />
                            Replace the {conflicts} record{conflicts === 1 ? "" : "s"} marked
                            someone else's
                        </label>
                    ) : null}
                    {results ? (
                        ...
                        <Button
                            ...
                            onClick={() => void apply()}
                            disabled={
                                !plan || applying || (changes === 0 && !(replace && conflicts > 0))
                            }
                        >
