 ...
 * server would refuse; and every page the left rail can reach has to be findable,
 * since both are drawn from the same registry.
 */
...
describe("navigationEntries", () => {
    ...
    it("withholds the admin app and its pages from a normal user", () => {
        ...
        expect(navigationEntries(true).map((entry) => entry.href)).toContain("/admin/users");
    });

    it("covers every section the left rail can reach", () => {
        const hrefs = new Set(navigationEntries(true).map((entry) => entry.href));
    ...
    it("carries an app's own name onto its landing section, so the app is searchable by it", () => {
        const files = navigationEntries(false).find((entry) => entry.href === "/drive");
