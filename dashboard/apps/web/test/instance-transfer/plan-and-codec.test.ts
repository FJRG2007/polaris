...
import { decodeRow, encodeRow, envelopeTriples, type SecretCodec } from "@/lib/instance-transfer/codec";
    ...
    it("exists for the schema as it is, with every required reference written first", () => {
        ...
        for (const model of real) {
            for (const field of model.fields) {
                ...
                const isDeferred = field.relationFromFields.every((column) => deferred.includes(column));
                if (isDeferred) {
                    // Only ever an optional column, written empty and filled in later.
                    for (const column of field.relationFromFields) {
                        expect(model.fields.find((candidate) => candidate.name === column)?.isRequired).toBe(false);
                    }
                    continue;
                }
                expect(position.get(field.type)!, `${model.name}.${field.name}`).toBeLessThanOrEqual(position.get(model.name)!);
            }
        }
    });
        ...
        const loop: SchemaModel[] = [
            {
                ...
                fields: [
                    ...
                    { name: "bId", kind: "scalar", type: "String", isRequired: true, isList: false },
                    { name: "b", kind: "object", type: "B", isRequired: true, isList: false, relationFromFields: ["bId"] }
                ]
            },
            {
                name: "B",
                fields: [
                    { name: "id", kind: "scalar", type: "String", isRequired: true, isList: false, isId: true },
                    { name: "aId", kind: "scalar", type: "String", isRequired: true, isList: false },
                    { name: "a", kind: "object", type: "A", isRequired: true, isList: false, relationFromFields: ["aId"] }
                ]
            }
        ];
...
function codecFor(key: string): SecretCodec {
    return {
        open: (blob) => (blob.keyId === key ? blob.ciphertext.toString("utf8").replace(`${key}:`, "") : null),
        seal: (plain) => ({ ciphertext: Buffer.from(`${key}:${plain}`), nonce: Buffer.from("n"), keyId: key }),
        openAuth: async (value) => (value.startsWith(`${key}!`) ? value.slice(key.length + 1) : null),
...
describe("a row on its way between instances", () => {
    it("finds the envelopes a table keeps in three columns", () => {
        expect(envelopeTriples(envVar)).toEqual([{ ciphertext: "encryptedValue", nonce: "valueNonce", keyId: "valueKeyId" }]);
    ...
    it("arrives re-sealed under the other instance's key", async () => {
        ...
        expect(Buffer.from(decoded.encryptedValue as Buffer).toString("utf8")).toBe("target:hunter2");
    ...
    it("carries a secret that does not open as it is, and counts it", async () => {
        const tally = { carried: 0, unreadable: 0 };
        const row = { encryptedValue: Buffer.from("x"), valueNonce: Buffer.from("n"), valueKeyId: "gone" };
        const decoded = await decodeRow(envVar, JSON.parse(JSON.stringify(await encodeRow(envVar, row, codecFor("source"), tally))), codecFor("target"));
    ...
    it("re-seals the sign-in library's two-factor secrets", async () => {
        const tally = { carried: 0, unreadable: 0 };
        const row = { id: "t1", secret: "source!TOTPSEED", backupCodes: "source![1,2]", verified: true };
        const wire = JSON.parse(JSON.stringify(await encodeRow(twoFactor, row, codecFor("source"), tally)));
    ...
    it("re-seals an envelope kept as one JSON string", async () => {
        ...
        const sealed = JSON.stringify({ c: Buffer.from("source:tok").toString("base64"), n: "bg==", k: "source" });
        const tally = { carried: 0, unreadable: 0 };
        const wire = JSON.parse(JSON.stringify(await encodeRow(setting, { key: "k", value: sealed }, codecFor("source"), tally)));
