...
describe("units", () => {
    it("judges a server's disk as a share and a service's as what it holds", () => {
        ...
        expect(describeThreshold("network_out", "application", "gt", 5)).toBe("Network out > 5 MB/s");
...
describe("readings", () => {
    ...
    it("has no rate without a reading close enough before", () => {
        ...
        expect(sampleValue("network_in", latest, reading("2026-09-10T10:00:00Z", { netRxBytes: 0n }))).toBeNull();
    });
    ...
    it("reads a server's disk as how full it is, and nothing where it is not measured", () => {
        const latest = reading("2026-09-10T10:00:00Z", { diskUsedBytes: 450n, diskTotalBytes: 500n });
    ...
    it("adds a service's volumes together, skipping the ones nothing could read", () => {
        expect(volumeDiskGb([{ diskUsedBytes: BigInt(GIB) }, { diskUsedBytes: BigInt(GIB / 2) }, { diskUsedBytes: null }])).toBe(1.5);
...
describe("the form's rules", () => {
    ...
    it("takes a disk alarm on a server, with a share as the threshold", () => {
        expect(alarmInputSchema.safeParse({ ...base, targetType: "host", metric: "disk", threshold: 90 }).success).toBe(true);
        expect(alarmInputSchema.safeParse({ ...base, targetType: "host", metric: "disk", threshold: 120 }).success).toBe(false);
    });

    it("takes a service's disk above 100, because it is GB rather than a share", () => {
        expect(
            alarmInputSchema.safeParse({ ...base, targetType: "application", metric: "disk", threshold: 250 }).success
        ).toBe(true);
    });

    it("asks for a threshold on a network alarm and refuses metrics a target has no reading for", () => {
        expect(alarmInputSchema.safeParse({ ...base, targetType: "host", metric: "network_in" }).success).toBe(false);
        expect(alarmInputSchema.safeParse({ ...base, targetType: "host", metric: "service" }).success).toBe(false);
        expect(alarmInputSchema.safeParse({ ...base, targetType: "domain", metric: "disk", threshold: 1 }).success).toBe(false);
        expect(alarmInputSchema.safeParse({ ...base, targetType: "domain", metric: "http" }).success).toBe(true);
    });
});
