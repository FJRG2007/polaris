...
describe("publishing with the operator's token", () => {
    ...
    it("narrows anybody else to the domain they verified", async () => {
        ...
        await expect(publishWithin(member, "org-1", "mail.example.com")).resolves.toBe(
            "example.com"
        );
    ...
    it("refuses a domain nobody on this shelf verified", async () => {
        await expect(publishWithin(member, null, "example.com")).rejects.toThrow(
...
describe("adding a mail domain", () => {
    it("refuses a domain on the operator's Cloudflare account the caller has not verified", async () => {
        await expect(requireMailDomainStanding(member, null, "example.com")).rejects.toThrow(
            /own DNS account/
        );
    });
    ...
    it("takes a domain the caller verified, or one hosted anywhere else", async () => {
        ...
        await expect(
            requireMailDomainStanding(member, null, "example.com")
        ).resolves.toBeUndefined();
        mocks.resolveZoneForHostname.mockRejectedValueOnce(
            new Error("not on a domain in this Cloudflare account")
        );
        await expect(
            requireMailDomainStanding(member, null, "elsewhere.org")
        ).resolves.toBeUndefined();
    });

    it("asks nothing of an administrator, and refuses nothing without a token", async () => {
        await expect(
            requireMailDomainStanding(admin, null, "example.com")
        ).resolves.toBeUndefined();
