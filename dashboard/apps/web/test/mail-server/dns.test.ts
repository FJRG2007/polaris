...
vi.mock("@polaris/db", () => ({
    prisma: { mailServer: { update: vi.fn(async () => undefined) } }
}));
...
vi.mock("@/lib/integrations/cloudflare-account-service", () => ({
    loadCloudflareToken: vi.fn(async () => "token")
}));
vi.mock("@/lib/integrations/cloudflare-api", () => ({
    ...
    listZoneRecords: vi.fn(
        async (_token: string, _zone: string, type: string, name: string) =>
            zoneRecords[`${type} ${name}`] ?? []
    ),
...
beforeEach(() => {
    ...
    zoneRecords["MX example.com"] = [
        { id: "mx", type: "MX", name: "example.com", content: "aspmx.l.google.com" }
    ];
    zoneRecords["TXT _dmarc.example.com"] = [
        { id: "dmarc", type: "TXT", name: "_dmarc.example.com", content: "v=DMARC1; p=none" }
    ];
});
...
describe("the plan", () => {
    it("adds to an existing SPF, keeps an existing DMARC, and leaves another MX alone", async () => {
        ...
        const by = (purpose: string) =>
            plan.records.find((entry) => entry.record.purpose === purpose);
        expect(by("spf")).toMatchObject({
            action: "update",
...
describe("whose zone it is", () => {
    it("refuses somebody who has not verified the domain, before the token reads the zone", async () => {
        ...
        await expect(dns.applyDns(member, server, "d1", true)).rejects.toThrow(
            /verified under Domains/
        );
    ...
    it("plans for somebody who verified the domain, or the organization the server is on", async () => {
        ...
        expect(plan.records.find((entry) => entry.record.purpose === "dkim")).toMatchObject({
            action: "create"
        });
...
describe("applying it", () => {
    ...
    it("reports a refusal as failed rather than done", async () => {
        failCreate = true;
        const results = await dns.applyDns(admin, server, "d1", false);
        expect(results.find((entry) => entry.name.includes("_domainkey"))).toMatchObject({
            outcome: "failed"
        });
    });
});
...
describe("what the resolvers answered", () => {
    it("reads each record type the way the zone writes it", async () => {
        const resolver = {
            ...
            resolveSrv: async () => [
                { priority: 0, weight: 1, port: 993, name: "mail.example.com" }
            ],
        ...
        expect(await dns.publishedValues(resolver, record("TXT"))).toEqual([
            "v=DKIM1; k=ed25519; p=abc"
        ]);
        expect(await dns.publishedValues(resolver, record("SRV"))).toEqual([
            "0 1 993 mail.example.com"
        ]);
        expect(await dns.publishedValues(resolver, record("CAA"))).toEqual([
    ...
    it("reads no such name as nothing published, and anything else as no answer", async () => {
        ...
        const down = {
            resolveMx: async () =>
                Promise.reject(Object.assign(new Error("x"), { code: "ETIMEOUT" }))
        } as never;
