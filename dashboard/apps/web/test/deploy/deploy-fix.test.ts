...
function written(): {
    ...
    const data = (
        db.application.update.mock.calls.at(-1) as unknown as [{ data: Record<string, string> }]
    )[0].data;
    return {
        sourceType: data.sourceType,
        source: JSON.parse(data.sourceConfig ?? "{}"),
        build: JSON.parse(data.buildConfig ?? "{}")
    };
}
...
describe("applying a fix", () => {
    it("sets the start command, keeps everything else, and deploys again", async () => {
        const next = await applyDeployFix("dep-1", "owner-1", "user-1", {
            kind: "set-start-command",
    ...
    it("clears a build command set to nothing", async () => {
        state.app = {
            ...state.app,
            buildConfig: JSON.stringify({ buildCommand: "npm run build" })
        };
        await applyDeployFix("dep-1", "owner-1", "user-1", {
            kind: "set-build-command",
            value: ""
        });
        expect(written().build).toEqual({});
    });
    ...
    it("generates Laravel's key in the shape Laravel reads, as a secret", async () => {
        ...
        expect(setEnvVar).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                key: "APP_KEY",
                isSecret: true,
                value: expect.stringMatching(/^base64:[A-Za-z0-9+/]{43}=$/)
            })
        );
    });
    ...
    it("keeps a typed credential secret and a typed setting plain", async () => {
        ...
        expect(setEnvVar).toHaveBeenLastCalledWith("application", "app-1", "owner-1", {
            key: "STRIPE_KEY",
            value: "sk_test_x",
            isSecret: true
        });
    ...
    it("moves a Dockerfile service with no Dockerfile onto the detected build", async () => {
        state.app = {
            ...
            sourceConfig: JSON.stringify({ repoUrl: "x", dockerfilePath: "Dockerfile" })
        };
    ...
    it("refuses a deploy that did not fail", async () => {
        state.status = "running";
        await expect(
            applyDeployFix("dep-1", "owner-1", "user-1", { kind: "set-port", port: 8080 })
        ).rejects.toThrow();
...
describe("what the screen may send", () => {
    it("refuses a command across lines, an empty start command and a variable with no value to set", () => {
        expect(
            deployFixInputSchema.safeParse({ kind: "set-start-command", value: "a\nrm -rf /" })
                .success
        ).toBe(false);
        expect(
            deployFixInputSchema.safeParse({ kind: "set-start-command", value: "  " }).success
        ).toBe(false);
        expect(
            deployFixInputSchema.safeParse({
                kind: "add-variable",
                name: "X",
                value: null,
                generate: false
        ...
        expect(
            deployFixInputSchema.safeParse({ kind: "set-runtime-version", version: "latest" })
                .success
        ).toBe(false);
