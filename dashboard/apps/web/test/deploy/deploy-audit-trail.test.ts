...
vi.mock("@/lib/deploy/references", () => ({
    resolveServiceReferences: async (env: Record<string, string>) => ({ env: { ...env }, unresolved: [] })
}));
...
const { deployApplication, restartFromKeptImage, rollbackToDeployment } = await import("@/lib/deploy-service");
...
describe("the audit entry a deploy writes", () => {
    it("is written once, crediting who asked, with the deployment and what started it", async () => {
        await deployApplication(APP, OWNER, "member-1", { trigger: "upload", audit: { via: "api", keyId: "key-1" } });
    ...
    it("credits nobody for a deploy nobody asked for, while the history still names the owner", async () => {
        ...
        expect(recordDeployAudit).toHaveBeenCalledWith(
            expect.objectContaining({ actorId: null, metadata: { deploymentId: "dep-1", trigger: "push" } })
        );
    ...
    it("carries the actor and the key through a restart of the live release", async () => {
        await restartFromKeptImage(APP, OWNER, "member-1", "variables", { via: "mcp", keyId: "key-2" });
        expect(recordDeployAudit).toHaveBeenCalledWith(
            expect.objectContaining({
                actorId: "member-1",
                metadata: { deploymentId: "dep-1", trigger: "variables", via: "mcp", keyId: "key-2" }
            })
        );
    });
