...
describe("presignAwsUrl", () => {
    ...
    it("signs the method, so a download link cannot be used to upload", () => {
        ...
        const put = new URL(presignAwsUrl({ ...EXAMPLE, method: "PUT" })).searchParams.get("X-Amz-Signature");
        expect(put).not.toBe(get);
    });

    it("encodes a key the way S3 signs it, keeping its slashes", () => {
        const url = presignAwsUrl({ ...EXAMPLE, protocol: "http", host: "files:8333", path: "/media/a b/(1)!.png" });
