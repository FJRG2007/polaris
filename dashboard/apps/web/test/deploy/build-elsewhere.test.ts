...
const { safeEntryPath, unpackSourceZip, uploadedSourceOf } = await import("@/lib/deploy/source-upload");
...
describe("where a build goes", () => {
    ...
    it("builds where it runs when nothing is chosen, or the choice is that machine", async () => {
        ...
        expect(
            await resolveBuildMachine({ buildConfig: JSON.stringify({ buildOn: "local" }), target: local }, "owner-1")
        ).toBeNull();
        expect(
            await resolveBuildMachine({ buildConfig: JSON.stringify({ buildOn: remote.hostId }), target: remote }, "owner-1")
        ).toBeNull();
    });

    it("builds on the Polaris host for a service that runs elsewhere", async () => {
        const machine = await resolveBuildMachine({ buildConfig: JSON.stringify({ buildOn: "local" }), target: remote }, "owner-1");
        expect(machine).toMatchObject({ target: { kind: "local" }, name: "the Polaris host", runsOn: "small-box" });
    });

    it("builds on another server the owner has connected", async () => {
        findFirstHost.mockResolvedValueOnce({ id: HOST, name: "big-box" });
        const machine = await resolveBuildMachine({ buildConfig: JSON.stringify({ buildOn: HOST }), target: local }, "owner-1");
        expect(machine).toMatchObject({ target: { kind: "host", hostId: HOST }, name: "big-box", runsOn: "the Polaris host" });
        expect(findFirstHost).toHaveBeenCalledWith(expect.objectContaining({ where: { id: HOST, ownerId: "owner-1" } }));
    });

    it("refuses in words when that server is no longer connected", async () => {
        findFirstHost.mockResolvedValueOnce(null);
        await expect(
            resolveBuildMachine({ buildConfig: JSON.stringify({ buildOn: HOST }), target: local }, "owner-1")
...
describe("an uploaded folder's paths", () => {
    ...
    it("refuses anything that climbs out, is absolute, or is left out", () => {
        for (const path of ["../etc/passwd", "a/../../b", "/etc/passwd", "C:/Windows/x", "node_modules/x/index.js", "a/.git/config", ".DS_Store"]) {
...
describe("unpacking an uploaded folder", () => {
    ...
    it("takes a zipped folder's contents as the root and leaves out what is skipped", async () => {
        ...
        const counted = await unpackSourceZip(await zip.generateAsync({ type: "nodebuffer", platform: "UNIX" }), dir);
    ...
    it("refuses what is not a zip, or holds nothing", async () => {
        ...
        await expect(unpackSourceZip(await empty.generateAsync({ type: "nodebuffer" }), dir)).rejects.toThrow("no files");
    });

    it("reads the upload a service remembers, and nothing that does not validate", () => {
        const upload = { id: HOST, name: "shop", files: 2, bytes: 30, uploadedAt: "2026-09-10T12:00:00.000Z" };
...
describe("an uploaded release", () => {
    const caller = { userId: "user-1", scopes: ["deploy.manage"], keyId: "key-1", projectId: null, via: "api" } as const;
    let dir = "";
    beforeEach(async () => {
    ...
    async function repository(): Promise<string> {
        ...
        return releaseImage(serviceName("shop", "web", "019f8506-683f-7dd0-9c13-1e9ee9237fe3"), "x").replace(/:[a-f0-9]{12}$/, "");
    }

    it("deploys one image under the service's own release repository as it is", async () => {
        const image = `${await repository()}:0123456789ab`;
        const result = await deployUploadedRelease(caller, "shop/web", await staged([image]), { commitSha: "abcdef1" });
        ...
        expect(deployApplication).toHaveBeenCalledWith(
            ...
            "owner-1",
            "user-1",
            expect.objectContaining({ trigger: "upload", commitSha: "abcdef1", prebuilt: expect.objectContaining({ image }) })
        );
    });

    it("refuses an archive that carries anything else", async () => {
        const image = `${await repository()}:0123456789ab`;
        await expect(deployUploadedRelease(caller, "shop/web", await staged([image, "traefik:v3"]), {})).rejects.toThrow(
            "must hold one image"
        );
        await expect(
            deployUploadedRelease(caller, "shop/web", await staged(["polaris-release/other:0123456789ab"]), {})
    ...
    it("refuses a tag that was deployed before", async () => {
        ...
        const image = `${await repository()}:0123456789ab`;
        await expect(deployUploadedRelease(caller, "shop/web", await staged([image]), {})).rejects.toThrow("deployed before");
    });
});
