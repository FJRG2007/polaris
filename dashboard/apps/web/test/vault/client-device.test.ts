...
describe("reading what a client is running in", () => {
    it("splits the browser from the system", () => {
        expect(readClientDevice("Brave on Windows")).toEqual({ browser: "Brave", os: "Windows" });
    });
    ...
    it("keeps the spelling the marks match on", () => {
        ...
        expect(readClientDevice("BRAVE on WINDOWS")).toEqual({ browser: "Brave", os: "Windows" });
        expect(readClientDevice("brave on macos")).toEqual({ browser: "Brave", os: "macOS" });
    });

    it("drops the word extension, which the column above already says", () => {
        expect(readClientDevice("Chrome extension")).toEqual({ browser: "Chrome", os: null });
    ...
    it("hands back an unfamiliar name rather than hiding it", () => {
        ...
        expect(readClientDevice("Bitwarden CLI")).toEqual({ browser: "Bitwarden CLI", os: null });
    });

    it("never comes back empty", () => {
        expect(readClientDevice("")).toEqual({ browser: "Unknown device", os: null });
        expect(readClientDevice("   ")).toEqual({ browser: "Unknown device", os: null });
    });

    it("does not split on a leading or trailing on", () => {
        expect(readClientDevice("on Windows").os).toBeNull();
        expect(readClientDevice("Chrome on ")).toEqual({ browser: "Chrome on", os: null });
    });
});
