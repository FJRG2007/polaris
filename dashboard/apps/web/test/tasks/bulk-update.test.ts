...
vi.mock("@polaris/db", () => ({
    prisma: {
        ...
        taskTagLink: { deleteMany: vi.fn(), createMany: vi.fn() },
...
vi.mock("../../src/lib/tasks/automation-service", () => ({ runAutomations, hasAutomationsFor }));
...
/** Which tasks a trigger was raised for. */
function raisedFor(trigger: string): string[] {
    return runAutomations.mock.calls
        .map(([event]) => event as { trigger: string; taskId: string })
