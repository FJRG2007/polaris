...
vi.mock("@polaris/db", () => ({
    prisma: { databaseOperation: { findFirst: mocks.findFirst, create: mocks.create, update: mocks.update } }
}));
...
describe("runStep", () => {
    it("names the step and never the command", async () => {
        const runtime = { runIn: vi.fn(async () => ({ code: 1, output: "FATAL: role does not exist" })) };
        await expect(
            ops.runStep(runtime as never, "db", { argv: ["psql", "-c", "PGPASSWORD=hunter22"], describe: "Loading the copy" }, [
                "hunter22"
            ])
...
describe("stageInto", () => {
    ...
    it("refuses a copy that did not arrive whole", async () => {
        ...
        const file = join(dir, "dump");
        await writeFile(file, "0123456789");
        await expect(ops.stageInto(ports("4\n") as never, "db", file, "/tmp/x.dump")).rejects.toThrow("4 of 10 bytes");
    });

    it("says so when the server cannot receive files", async () => {
        await expect(ops.stageInto({ runIn: vi.fn() } as never, "db", "/nowhere", "/tmp/x")).rejects.toThrow(
...
describe("startOperation", () => {
    ...
    it("refuses a second operation while one is running", async () => {
        ...
        await expect(ops.startOperation("db-1", "copy", "user")).rejects.toThrow("A restore is already running");
        expect(mocks.create).not.toHaveBeenCalled();
    });

    it("treats one left running for six hours as abandoned", async () => {
        mocks.findFirst.mockResolvedValue({ kind: "restore", startedAt: new Date(Date.now() - 7 * 3_600_000) });
        await expect(ops.startOperation("db-1", "copy", "user")).resolves.toMatchObject({ id: "op-1" });
    });
    ...
    it("records an unexpected failure in general words and a refusal in its own", async () => {
        ...
        expect(await operation.fail(new Error("ECONNREFUSED 10.0.0.5:2375"))).toMatch(/did not expect/);
        expect(await operation.fail(new ops.DatabaseOperationError("Version 18 did not start."))).toBe(
            "Version 18 did not start."
        );
        log.mockRestore();
    });
});
