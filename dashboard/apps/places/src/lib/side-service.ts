...
import { tunnelledUrl } from "./tunnel";
import { host } from "@polaris/app-host";

const { listHosts } = host.hostService;
const { localDialHost } = host.deployDial;
const { findApp } = host.appsCatalog;
const { serviceRef } = host.deployReleases;
...
async function containerUrl(applicationId: string): Promise<string | null> {
    ...
    if (!port) return null;
    const { name } = await serviceRef(
        application.environment.project.slug,
        application.slug,
        application.id
    );
