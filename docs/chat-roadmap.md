...
| A DM share arrives as a card, not taken over | ✅     | -    | -    | Direct messages only - a room or channel call still opens a share straight away, because it has the room to spare. Arrives as a card beside the faces; one press opens it, and watching takes the band with the faces moved to a strip until it closes. Never offered back to whoever just pressed Share. See `call-room.tsx`, `call-media.ts`. Discord's fullscreen-with-people-underneath is not this - that is fullscreen on the call rather than on the tile, and is a separate change. |
