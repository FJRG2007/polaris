...
| A link to Polaris renders as the thing | ✅ | P1 | M | A conversation pasted into a conversation is a `#name`, a voice room is a card with who is in it now and a way in, a message is the message, a task is its chip. Resolved per reader on every read, never stamped into the message: who is in a voice room is true for a minute, and a name is the reader's to be allowed or not. Out of reach carries nothing at all - not the name, not the excerpt. |
