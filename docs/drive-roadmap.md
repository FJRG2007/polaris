...
- **Quota is an account attribute**, and what counts against it is what the
  account _owns_: files shared in from somebody else count against the sharer,
...
browsed as locations; **a private drive per account**; **a drive per
organization**, reached by everybody on its roster and written to by whoever
holds `drive.manage`, with who uploaded each item still recorded per person;
**sending a file or a folder to a person or an organization**, a copy by
default or the item itself on a move, which leaves the sender only once the
recipient accepts; **sharing an item with a person or a group, with a role, a
note and an expiry, and both directions listed on one screen**; public links
with a password, an expiry, a download cap, an
...
| --------------------- | ------ | ---- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **WebDAV endpoint**   | ⬜     | P0   | L    | Polaris is a WebDAV _client_ and not a _server_. Serving one is what makes every existing client work at once - Finder, Windows Explorer, GNOME Files, rclone, mobile file managers - without writing any of them. Do this first. |
...
| Mounting a share in your own tree | ⬜     | P2   | M    | Nextcloud's `file_target`: a shared folder appears _inside_ your home rather than on a separate screen. This is the part that makes shared items feel like your own files.                                                           |
| Group folders (team drives)       | ✅     | P1   | L    | Every organization gets its own drive, made the first time somebody on its roster opens it. Reading is roster-wide; writing needs `drive.manage`. Team-scoped (rather than org-wide) folders are not separately modelled - Nextcloud's per-group ACL on a groupfolder still does not exist.  |
